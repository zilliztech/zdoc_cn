...
notebook: false
description: "SentenceTransformerEmbeddingFunction 中的此操作接收文本字符串列表，并将其直接编码为向量嵌入。 | Python"
type: docx
...
[SentenceTransformerEmbeddingFunction](./EmbeddingModels-SentenceTransformerEmbeddingFunction) 中的此操作接收文本字符串列表，并将其直接编码为向量嵌入。

为避免直接使用 **\_\_call()\_\_** 方法时可能出现的错误，请避免在初始化 SentenceTransformerEmbeddingFunction 时使用 **query_instruction** 或 **doc_instruction**。更多信息，请参阅 [SentenceTransformerEmbeddingFunction](./EmbeddingModels-SentenceTransformerEmbeddingFunction)。
