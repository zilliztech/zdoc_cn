...
notebook: FALSE
description: "访问日志以 JSON Lines 格式传输——每行一个 JSON 对象。每行都是一个独立的 JSON 对象，代表一次操作。以下示例展示了一个 Search 操作，为便于阅读进行了格式化： | Cloud"
type: origin
...
访问日志以 [JSON Lines](https://jsonlines.org/) 格式传输——每行一个 JSON 对象。每行都是一个独立的 JSON 对象，代表一次操作。以下示例展示了一个 Search 操作，为便于阅读进行了格式化：
