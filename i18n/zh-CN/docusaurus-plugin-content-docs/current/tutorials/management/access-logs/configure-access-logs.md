...
notebook: FALSE
description: "本指南涵盖 Zilliz Cloud 集群上访问日志的完整生命周期：启用、调整设置和停用。 | Cloud"
type: origin
...
1. 在弹出的对话框中，配置以下选项：
    ...
    - **目录**：指定存储桶中用于存放访问日志的目录路径。

    - **采样率**：设置要记录的查询比例。100% 表示记录每一次操作。对于高负载工作负载，可以选择较低的比例（例如 1%），在保持统计意义的同时降低存储成本。
...
1. 点击**保存**以应用更改。更新后的设置会立即对新日志记录生效。存储桶中已有的日志文件不受影响。
...
1. 点击**停用**。新的日志记录会立即停止。存储桶中已有的日志文件将保留。确认后，访问日志的计费随即停止。
