...
notebook: FALSE
description: "审计日志允许管理员跟踪和监控 Zilliz Cloud 集群上的用户驱动的操作和 API 调用。此功能提供数据平面活动的详细记录，包括向量搜索、查询执行、索引管理和其他数据操作。 | Cloud"
type: origin
...
审计日志允许管理员跟踪和监控 Zilliz Cloud 集群上的用户驱动的操作和 API 调用。此功能提供数据平面活动的详细记录，包括向量搜索、查询执行、索引管理和其他数据操作。

<Admonition type="info" icon="📘" title="说明">

- 审计日志功能仅对 **Dedicated** 集群可见。如有需要，请考虑[升级集群](./manage-cluster)。

- 仅 Milvus 2.5.x 及以上版本的集群支持审计日志功能。
...
迁移、备份等数据任务以及恢复等集群操作不会生成集群审计日志。您可以前往组织事件页面查看相关操作记录。具体请参考[查看事件](./view-activities)。
...
- **文件命名规则**：具体格式为 `<File name><File name suffix>`。`<File name>` 的格式为 *HH:MM:SS-&#36;UUID*：*HH:MM:SS* 表示日志生成时的 UTC 时间戳；*&#36;UUID* 表示一个随机字符串，例如 `09:16:53-jz5l7D8Q`。
...
有关审计日志支持的操作及对应字段，请参阅[审计日志参考](./audit-logs-ref)。
...
- 您的 Zilliz Cloud 集群版本为 **Dedicated** 及以上。如有需要，请考虑[升级集群](./manage-cluster)。

- 您已经为 Zilliz Cloud 项目配置了对象存储集成，因为配置完成后审计日志会转发到您的存储桶。有关详细步骤，请参阅[阿里云对象存储](./integrate-with-alibaba-cloud-oss)或 [Amazon S3](./integrate-with-amazon-s3-cn)。
...
1. 进入目标集群的详情页面，选择**日志**选项卡。当集群处于创建中、删除中或已删除状态时，该选项卡不可用。
...
- **异常集群状态会影响审计日志服务吗？**
    ...
    集群状态异常意味着集群可能存在网络连接或 Zilliz Cloud 服务中断等问题。然而，这些问题不会影响审计日志服务，该服务会继续正常运行并将日志转发到您的存储桶。如果您遇到持续性问题，请联系 [Zilliz Cloud 支持](https://zilliz.com.cn/contact-sales)。
