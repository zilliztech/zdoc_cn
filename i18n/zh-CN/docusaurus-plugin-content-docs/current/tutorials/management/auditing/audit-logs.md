...
notebook: FALSE
description: "审计日志允许管理员跟踪和监控 Zilliz Cloud 集群上的用户驱动操作和 API 调用。此功能提供数据平面活动的详细记录，包括向量搜索、查询执行、索引管理以及其他数据操作。 | Cloud"
type: origin
...
审计日志允许管理员跟踪和监控 Zilliz Cloud 集群上的用户驱动操作和 API 调用。此功能提供数据平面活动的详细记录，包括向量搜索、查询执行、索引管理以及其他数据操作。
...
有关审计日志支持的操作及其对应字段，请参阅[审计日志参考](./audit-logs-ref)。
