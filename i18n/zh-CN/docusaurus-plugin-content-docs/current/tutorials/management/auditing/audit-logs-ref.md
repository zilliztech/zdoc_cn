...
<table>
   ...
   <tr>
     ...
     <td><p>String</p></td>
     <td><p>涉及的数据库名称（例如 <code>"default"</code>）。</p></td>
   </tr>
   <tr>
...
| GetCollectionStatistics | 获取 Collection 的统计信息（例如 Collection 中的 Entity 数量） |
| Flush | 将 Collection 中的所有 Entity 持久化至 Sealed Segment。在 Flush 操作之后插入的任何 Entity 都会存储到新的 Segment 中。 |
