...
登录 Zilliz Cloud，并在左侧导航栏顶部的下拉菜单中选择目标项目。
...
![zh-amazon-authorize-private-endpoint](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/zh-amazon-authorize-private-endpoint.png "zh-amazon-authorize-private-endpoint")
...
1. **使用 Amazon Route 53 创建一个托管区域**
    ...
    Amazon Route 53 是一个基于 Web 的 DNS 服务。您需要创建一个 DNS 托管区域，以便向其中添加 DNS 记录。
    ...
    1. 在托管区域配置部分，设置以下参数。
        ...
        | 参数名称 | 参数说明 |
        | --- | --- |
        | 域名 | 这是您要将流量路由到其中的域的名称。<br/>此处应填写 Zilliz Cloud 为目标集群分配的私网连接。 |
        | 描述 | 您可以使用此值来区分具有相同名称的托管区。 |
        | 类型 | 类型说明了您希望通过互联网还是在 Amazon VPC 中路由流量。<br/>您应该选择私有托管区。 |
