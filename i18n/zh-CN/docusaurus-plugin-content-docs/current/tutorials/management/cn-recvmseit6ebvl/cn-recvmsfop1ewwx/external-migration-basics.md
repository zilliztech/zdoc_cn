...
| 数据源 | 类型 | 主要功能 |
| --- | --- | --- |
| [Qdrant](./migrate-from-qdrant) | 向量数据库 | 开源引擎，支持云部署与自托管 |
| [Elasticsearch](./migrate-from-elasticsearch) | 搜索引擎 | 支持稠密向量与全文检索 |
| [PostgreSQL](./migrate-from-pgvector) | 关系型数据库 | 支持向量扩展（pgvector） |
| [腾讯云向量数据库](./migrate-from-tencent-cloud) | 托管服务 | 全托管向量数据库服务 |
| [OpenSearch](./migrate-from-opensearch) | 搜索平台 | 支持向量检索的 KNN 插件 |
...
<table>
   ...
   <tr>
     <td><p>添加额外字段</p></td>
     <td><p>扩展源数据之外的字段，以适应业务演进需求</p></td>
   </tr>
   <tr>
     <td><p>数据类型映射</p></td>
   ...
   <tr>
     ...
     <td><p>智能命名处理</p></td>
     <td><p>默认保留源表名；重名时系统报错；含连字符(-)的名称会自动转为下划线(_)或提示修正</p></td>
   </tr>
   <tr>
     <td><p>Shard 配置</p></td>
   ...
   <tr>
     ...
     <td><p>在迁移过程中为文本字段开启 Full Text Search。</p></td>
     <td><p>可在 <strong>高级设置 → Function</strong> 中配置，以便在迁移期间为 <strong>VARCHAR</strong> 字段启用 <strong>Full Text Search</strong>。如果源数据的元数据包含文本，可使用<strong>转换为固定字段</strong>将文本型元数据转换为 <strong>VARCHAR</strong> 字段。详情参见 <a href="./full-text-search">Full Text Search</a>。</p></td>
   </tr>
</table>
...
![MPbQwTCqBhybmZbzfAfc0Xnenab](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/MPbQwTCqBhybmZbzfAfc0Xnenab.png)

### 阶段 1：连接与配置\{#phase-1-connect-and-configure}

1. 建立连接：提供源系统认证凭证（API 密钥/连接字符串）
...
- [从腾讯云向量数据库迁移至 Zilliz Cloud](./migrate-from-tencent-cloud)
