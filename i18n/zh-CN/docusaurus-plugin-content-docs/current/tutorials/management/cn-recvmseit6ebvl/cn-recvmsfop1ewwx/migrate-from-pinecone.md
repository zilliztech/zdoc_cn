...
- 在将数据从 Pinecone 迁移至 Zilliz Cloud 时，向量字段会直接迁移，而 Pinecone 的元数据字段将以 JSON 格式存储在 Zilliz Cloud 的动态字段中。有关动态字段功能的详细信息，请参阅[动态字段](./enable-dynamic-field)。
...
1. 在**连接数据源**步骤中，输入访问目标 Pinecone 项目的 API 密钥，然后点击**下一步**。
    ...
    阅读 [身份验证](https://docs.pinecone.io/reference/api/authentication) 文档将帮助您获取所需的连接信息。
