...
集群可创建的 Collection 数量受集群类型和 CU 数量影响。

| **类型** | **最大数量** | **描述** |
| --- | --- | --- |
| Free 版集群 | 5 | 在 Free 版集群中，最多可创建 5 个 Collection。 |
| Serverless 版集群 | 100 | 在 Serverless 版集群中，最多可创建 100 个 Collection。 |
| Dedicated 版集群 | 每 CU：&lt;= 64<br/>每集群：&lt;= 4096 | 在 Dedicated 版集群中，每个计算单元（CU）最多可创建 64 个 Collection，并且集群中的 Collection 总数不能超过 4096。 |

更多详情，请参见[使用限制](./limits#collections)。
...
Zilliz Cloud 支持以下几种相似度类型。

1. 欧氏距离（Euclidean / L2）计算两条向量间的欧氏距离。计算结果越小，两个向量越相似。

1. 内积（Inner Product / IP）将两条向量相乘。计算结果为正数，两个向量越相似。

1. 余弦距离（Cosine）计算两个向量之间的余弦夹角大小。计算结果在 [-1, 1] 的区间内，结果越大，向量越相似。
...
您可以使用我们的 PyMilvus SDK 通过提供参数 collection.ttl.seconds 的值来设置 collection 的 TTL。

以下示例将 TTL 设置为 1800 秒。
...
是。Partition 名称只能包含字母、数字、下划线（“_”）和连字符（“-”），且不能以数字或连字符开头。
