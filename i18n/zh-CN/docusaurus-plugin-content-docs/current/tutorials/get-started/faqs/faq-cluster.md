...
- 连接参数不正确
    ...
    Zilliz Cloud 集群已启用 TLS。请确保在连接参数中加入 `secure`，并将其设置为 `true`。未正确设置该参数可能导致连接失败和超时。
...
1. 确认 Endpoint URI 中的端口是否正确。请确保您从 Zilliz Cloud Web 控制台中复制正确的 Endpoint。下表罗列了部署在不同云服务提供商的集群对应端口。
