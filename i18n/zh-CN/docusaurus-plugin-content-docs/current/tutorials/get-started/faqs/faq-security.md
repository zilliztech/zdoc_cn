...
beta: FALSE
notebook: FALSE
description: "本文涵盖了在使用 Zilliz Cloud 平台期间与您的数据安全相关的问题，包括证书授权机构、证书有效期、检查证书是否过期的相关步骤、支持的传输层安全协议（TLS）版本及相应的鉴权方法。 | CLOUD"
type: origin
...
本文涵盖了在使用 Zilliz Cloud 平台期间与您的数据安全相关的问题，包括证书授权机构、证书有效期、检查证书是否过期的相关步骤、支持的传输层安全协议（TLS）版本及相应的鉴权方法。
...
- [Zilliz Cloud 支持哪些 TLS 版本？](#what-tls-versions-are-supported-on-zilliz-cloud)
...
您可以使用如下命令检查您的 Zilliz Cloud 集群证书是否已过期。如下命令假设您的集群托管在阿里云上，其实例 ID 为 `inxx-xxxxxxxxxxxxxxxxx`。请确保目标实例 URL 以正确的端口号结尾，如 `:443`。
