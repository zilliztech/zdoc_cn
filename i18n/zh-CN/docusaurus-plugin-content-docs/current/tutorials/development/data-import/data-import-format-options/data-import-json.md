---
title: "从 JSON/JSON Lines 文件中导入 | Cloud"
...
notebook: FALSE
description: "JSON 文件是一种轻量级、对人和机器都友好的数据格式。由于它独立于语言，并遵循 C 系语言程序员熟悉的约定，JSON 是一种理想的数据交换格式。 | Cloud"
type: origin
...
[JSON](https://www.json.org/json-en.html) 文件是一种轻量级、对人和机器都友好的数据格式。由于它独立于语言，并遵循 C 系语言程序员熟悉的约定，JSON 是一种理想的数据交换格式。

JSON Lines 是一种文本格式，每一行都是一个完整且合法的 JSON 对象，因此可以使用标准文本工具以增量方式处理数据流。
...
- **是否启用 AutoID**
    ...
    Collection 中的 id 字段用作区分 Entity 的主键。如果需要 Zilliz Cloud 为插入的数据自动生成主键，可以启用 AutoID。在这种情况下，批量导入数据时需要排除主键列。

- **是否启用 Dynamic Field**

    当目标 Collection 启用了 Dynamic Field，并且您希望导入 Schema 中未定义的字段时，可以在导入数据中包含一个名为 **&#36;meta** 的字段，并将所有未在 Schema 中定义的字段以键值对形式存放到 **&#36;meta** 字段中。

- **大小写**

    字典键名和 Collection 字段名区分大小写。您需要确保待导入字典的键名与目标 Collection 的字段名完全一致。例如，如果目标 Collection 中有一个名为 **id** 的字段，则待导入字典的键名也应为 **id**。使用 **ID** 或 **Id** 会导致错误。
...
准备好待导入数据后，您可以使用以下任一方式将其导入到您在 Zilliz Cloud 上创建的 Collection 中。

- [从多个路径导入（推荐）](./data-import-json#import-files-from-multiple-paths-recommended)
...
如果文件体积较小，建议使用多路径或源文件夹方式一次性导入所有文件。Zilliz Cloud 针对这两种方式进行了内部优化，可以降低后续资源消耗。

</Admonition>

您也可以使用 Zilliz Cloud 控制台或 Milvus SDK。具体操作请参阅[通过 Web 控制台导入](./import-data-on-web-ui)和[通过 SDK 导入](./import-data-via-sdks)。

### 从多个路径导入（推荐）\{#import-files-from-multiple-paths-recommended}

当您有多个文件需要同时导入时，可以使用这种方式。导入时，需要将每个待导入 JSON 文件的路径包含在一个子列表中，并将这些子列表包含在一个外层列表中，如下方代码所示。
...
如果源文件夹中只包含所有待导入文件，您可以将该源文件夹的路径放在一个子列表中，然后将这个子列表放入外层列表，如下方代码所示。
...
如果您希望单独导入一个 JSON 文件，可以将该 JSON 文件的路径放在一个子列表中，然后将这个子列表放入外层列表，如下方代码所示。
...
请注意，合法的 JSON 文件中包含一个名为 **rows** 的根键，其值为字典列表，每个字典代表一个要插入的 Entity 对象。
...
建议您[使用 BulkWriter](./use-bulkwriter) 工具将原始数据转换为 JSON 文件。我们根据本文示意图中的 Schema 准备了一份示例数据供您参考。[单击此处](https://assets.zilliz.com/prepared_json_data.json)下载该示例文件。
