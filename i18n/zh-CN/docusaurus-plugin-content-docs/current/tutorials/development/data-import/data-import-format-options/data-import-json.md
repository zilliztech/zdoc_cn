...
notebook: FALSE
description: "JSON 文件是一种轻量级、对人和机器都友好的数据格式。由于其语言无关的特性，并遵循类 C 语言程序员熟悉的规范，JSON 是一种理想的数据交换格式。 | Cloud"
type: origin
...
[JSON](https://www.json.org/json-en.html) 文件是一种轻量级、对人和机器都友好的数据格式。由于其语言无关的特性，并遵循类 C 语言程序员熟悉的规范，JSON 是一种理想的数据交换格式。

JSON Lines 是一种文本格式，其中每一行都是一个完整且合法的 JSON 对象，因此可以使用标准文本工具以增量方式处理数据流。
...
- **大小写**
    ...
    字典键名和 Collection 字段名区分大小写。您需要确保待导入字典的键名与目标 Collection 的字段名完全对应。例如，目标 Collection 中有一个名为 **id** 的字段，则待导入字典的键名也应为 **id**。使用 **ID** 或 **Id** 会导致报错。

</Admonition>

## 目录结构\{#directory-structure}

如果您希望将原始数据转换为 JSON 或 JSON Lines 文件，请将所有文件直接放到源文件夹中，如下所示。
...
准备好待导入数据后，您可以使用以下任意一种方式将它们导入到您在 Zilliz Cloud 上创建的 Collection 中。
...
如果文件体积较小，建议使用多路径或源文件夹方式一次性导入所有文件。Zilliz Cloud 针对这两种方式做了内部优化，可以降低后续资源消耗。

</Admonition>

您也可以使用 Zilliz Cloud 控制台或 Milvus SDK。具体操作可参考[通过 Web 控制台导入](./import-data-on-web-ui)以及[通过 SDK 导入](./import-data-via-sdks)。

### 从多路径导入（推荐）\{#import-files-from-multiple-paths-recommended}

当您有多个文件需要同时导入时，可以使用这种方式。在导入时，需要将每个待导入 JSON 文件的路径包含在一个子列表中，并将这些子列表包含在一个外层列表中，如下方代码所示。
...
如果源文件夹中仅包含所有需要导入的文件，可以将该源文件夹的路径放在一个子列表中，然后将该子列表放在一个外层列表中，如下方代码所示。
...
如果您希望单独导入一个 JSON 文件，可以将该 JSON 文件的路径放在一个子列表中，然后将该子列表放在一个外层列表中，如下方代码所示。
...
请注意，一个合法的 JSON 文件中有一个名为 **rows** 的根键，其值为一个字典列表，其中每个字典代表一个需要插入的 Entity 对象。
