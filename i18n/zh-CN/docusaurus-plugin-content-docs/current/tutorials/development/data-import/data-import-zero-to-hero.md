...
sidebar_label: "数据导入指南"
beta: FALSE
added_since: FALSE
last_modified: FALSE
deprecate_since: FALSE
notebook: FALSE
description: "本小节为快速入门教程，旨在帮助您在 Zilliz Cloud 上快速开始端到端的数据导入任务，包括数据准备、创建 Collection 和开始导入数据等子任务。通过本节，您将掌握： | Cloud"
type: origin
...
本小节为快速入门教程，旨在帮助您在 Zilliz Cloud 上快速开始端到端的数据导入任务，包括数据准备、创建 Collection 和开始导入数据等子任务。通过本节，您将掌握：

- 如何定义 Schema 及创建导入任务的目标 Collection

- 如何使用 **BulkWriter** 准备源数据并将其写入远程对象存储桶

- 如何调用批量导入 API 将准备好的源数据导入目标 Collection

<Admonition type="info" icon="📘" title="说明">

Zilliz Cloud 允许您将数据从任意对象存储导入到任意 Zilliz Cloud 集群，不用考虑托管集群的云服务商。例如，您可以将存储在阿里云上的数据导入到部署在腾讯云上的集群。

但是，Zilliz Cloud 仍建议您从与目标集群位于相同云服务商相同云地域的对象存储桶中导入数据，以获得低时延、高稳定的数据操作体验。
...
- 如果您还未创建任何集群，请参考[此处创建一个集群](./create-cluster)。
...
在终端中运行以下命令安装 **pymilvus** 和 **minio**，或将它们升级到最新版本。
...
- 如您使用 Apache Maven 管理项目，请在项目的 **pom.xml** 文件中添加如下内容：
...
- 如您使用 Gradle/Grails 管理项目，请执行如下命令：
...
为了更好地演示 Collection 的能力，我们在目标 Collection 的 Schema 中包含了前 4 个字段，并将后 4 个字段作为动态字段使用。
...
当 Schema 准备好后，就可以使用该 Schema 创建 **RemoteBulkWriter** 了。由于 **RemoteBulkWriter** 需要访问您的远程对象存储桶，因此您需要先设置好连接远程对象存储桶的 **ConnectParam** 对象，并在创建 **RemoteBulkWriter** 时引用该参数。
...
- `segment_size=512*1024*1024`
    ...
    此参数决定了 **BulkWriter** 如何对原始数据进行分段。该参数默认值为 512 MB (512 * 1024 * 1024)。如果您的数据集包含的数据量较大，可以考虑使用该参数对数据进行合理分段。
...
- `withChunkSize(512*1024*1024)`
    ...
    此参数决定了 **BulkWriter** 如何对原始数据进行分段。该参数默认值为 512 MB (512 * 1024 * 1024)。如果您的数据集包含的数据量较大，可以考虑使用该方法对数据进行合理分段。
