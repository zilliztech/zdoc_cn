...
notebook: FALSE
description: "本文展示了如何统计 Collection 中的 Entity 数量，并解释了 Entity 计数可能与实际数量不同的原因。 | Cloud"
type: origin
...
本文展示了如何统计 Collection 中的 Entity 数量，并解释了 Entity 计数可能与实际数量不同的原因。

## 概述\{#overview}

Zilliz Cloud 为您提供了两种统计 Collection 中 Entity 数量的方法。

- **查询并将 count(&ast;) 作为输出字段**

    要获取 Collection 中 Entity 的确切数量，您应使用此方法并确保：
    ...
    在收到此类查询时，Zilliz Cloud 会向查询节点发送请求，并统计已加载到内存中的 Entity 数量。

    您可以在查询中指定多个 Partition 名称，以获取这些 Partition 中对应的 Entity 计数。有关详细信息，请参阅[以 count(*) 作为输出字段进行查询](./count-entities#query-with-count-as-the-output-field)。

- **使用 get_collection_stats()**

    虽然您可以使用上述方法获取 Collection 的确切计数，但不建议在所有场景中都使用它。此过程本质上是一次查询，频繁调用可能会导致网络抖动，或者影响与您业务相关的搜索和查询。

    如果统计结果的准确性不是主要考虑因素，您应使用 `get_collection_stats()` 和 `get_partition_stats()`。虽然此调用提供的是估算的 Entity 计数，但您无需加载目标 Collection 即可运行它，而且成本小到可以忽略不计，因为它只是报告内部跟踪器记录的内容。
...
除了通过编程方式获取 Entity 计数外，您还可以在 Zilliz Cloud 控制台中查看集群、Collection 或 Partition 的相关数据。您可以阅读 [Zilliz Cloud 控制台中的 Entity 计数](./count-entities#entity-counts-on-the-zilliz-cloud-console)以了解详细信息。

## 以 `count(*)` 作为输出字段进行查询\{#query-with-count-as-the-output-field}

要获取准确的 Entity 计数，请加载 Collection 并运行一个查询，将 `count(*)` 作为输出字段，且将查询的 `consistency_level` 设置为 `Strong`。
...
如上文所述，`get_collection_stats()` 返回 Collection 中 Entity 的估算数量，该数量可能与实际 Entity 计数不同。您可以将其作为参考，而无需加载 Collection。
...
您可以在 Collection 的详情选项卡中找到其实际 Entity 计数。此值是通过[查询并将](./count-entities#query-with-count-as-the-output-field)[`count(*)`](./count-entities#query-with-count-as-the-output-field)[作为输出字段](./count-entities#query-with-count-as-the-output-field)获得的。
...
您还可以使用 Collection 的**分区**选项卡来查找其子分区中已加载实体的估算数量。此值通过使用 `get_partition_stats()` 获得。
...
- **为什么在我插入一些 Entity 后，使用 get_collection_stats() 或 get_partition_stats() 获取的 Entity 计数没有反映目标 Collection 或 Partition 中的实际 Entity 数量？**

    这些方法仅报告内部跟踪器记录的内容，由于所有数据操作都是异步的，因此可能与实际 Entity 计数不同。

- **在我插入或删除一些 Entity 后，Collection 的指标选项卡上的实体计数曲线为何没有变化？**

    **实体计数**曲线中的值是在特定时间点估算的，由于所有数据操作都是异步的，因此在它们反映到曲线中之前可能会有延迟。

- **为什么在 Collection 的“分区”选项卡的“实体计数（近似）”列中显示的值在我插入或删除一些 Entity 后没有改变？**

    列出的分区所显示的值均为估算值。由于所有数据操作都是异步的，因此在它们反映到曲线中之前可能会有延迟。

- **为什么 Collection 概览选项卡中“已加载实体”显示的值不能反映 Collection 中 Entity 的实际数量？**

    **已加载实体**中显示的值是准确的。如果此值与从典型查询获得的 Entity 数量之间存在差距，则 Collection 中的某些 Entity 可能具有相同的主键。

    请注意，以 `count(*)` 作为输出字段的查询会将具有相同主键的 Entity 视为不同 Entity，而其他查询会在返回最终结果之前省略具有相同主键的 Entity。
