...
notebook: FALSE
description: "在 Zilliz Cloud 中使用 `STL_SORT` 索引，可以将 `INT8` 与 `INT16` 等数值类型字段、`VARCHAR` 字段或 `TIMESTAMPTZ` 字段中的值按既定方式排序，从而提升在这些字段上进行查询时的性能。 | Cloud"
type: origin
...
在 Zilliz Cloud 中使用 `STL_SORT` 索引，可以将 `INT8` 与 `INT16` 等数值类型字段、`VARCHAR` 字段或 `TIMESTAMPTZ` 字段中的值按既定方式排序，从而提升在这些字段上进行查询时的性能。
...
使用 `STL_SORT` 索引时，Zilliz Cloud 会按如下步骤创建索引并加速查询：
...
1. 加速查询
    ...
    1. 在该字段中包含匹配值的 Entity 主键会被传递给查询执行器，用于组装最终查询结果。

上述步骤可将查询复杂度由 **O(n)** 降低到 **O(logn +m)**，其中 m 表示匹配 Entity 的数量。

## 创建 STL_SORT 索引\{#create-an-stl_sort-index}

您可以直接在数值、`VARCHAR` 或 `TIMESTAMPTZ` 字段上使用 `STL_SORT` 索引，无需额外参数。如下示例演示了如何在 `TIMESTAMPTZ` 字段上使用 `STL_SORT` 索引。
