...
在 Zilliz Cloud 中，过滤搜索分为两种类型，即**标准过滤搜索**和**迭代过滤搜索**。这两种类型的区别在于何时进行标量过滤。

### 标准过滤搜索\{#standard-filtering}
...
![MiVRw4H1KhPxAVb6L5icIczTnCc](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/MiVRw4H1KhPxAVb6L5icIczTnCc.png)
...
### 迭代过滤搜索\{#iterative-filtering}

标准过滤搜索的标量过滤过程有效地将搜索限定在一个较小的范围。然而，过于复杂的过滤表达式可能会导致非常高的搜索延迟。在这种情况下，迭代过滤搜索可以作为一种替代方案，有助于减少标量过滤的工作量。

![XSNPwbb6uhqnlqb9EXwcOybhnae](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/XSNPwbb6uhqnlqb9EXwcOybhnae.png)

如上图所示，一个迭代过滤搜索请求会将搜索过程分成多次迭代进行。在每次迭代中，会先进行向量搜索，然后再进行标量过滤，并去除掉本次迭代中不满足标量过滤条件的结果。当 topK 满足后，返回最终结果。

这种方式显著地减少了标量过滤需要处理的 Entity 数量，使得其尤其适用于处理过滤条件较为复杂的场景。

需要注意的是，迭代器每次只处理一个 Entity，并以串行的方式进行迭代。使用迭代过滤搜索可能会拉长请求处理时间或在需要处理的 Entity 较大时出现潜在的性能问题。

## 操作示例\{#examples}

本节将结合具体的示例介绍如何进行过滤搜索。在如下示例中，假设 Collection 已经存放了如下 10 条Entity。每个 Entity 都有 **id**, **vector**, **color** 和 **likes** 这几个字段。
...
### 使用标准过滤搜索\{#search-with-standard-filtering}
...
### 使用迭代过滤搜索\{#search-with-iterative-filtering}

要使用迭代过滤搜索进行过滤搜索，您可以执行以下操作：
