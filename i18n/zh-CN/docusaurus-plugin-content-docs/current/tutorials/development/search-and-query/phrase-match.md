...
- `slop`（可选）：一个整数，指定匹配标记中允许的最大位置数。
    ...
    - `2`：允许更多灵活性，包括词序颠倒或中间最多两个词。示例：匹配目标为 **"machine learning"** 的过滤表达式将匹配 **"learning machine"**（词序颠倒）或 **"machine quickly boosts learning"**（**"machine"** 和 **"learning"** 之间有两个词）。
...
此示例返回包含精确短语 **"machine learning"** 且中间没有任何额外内容的文档。
...
在搜索操作中，**PHRASE_MATCH** 用于在应用向量相似度排序之前对文档进行预过滤。这种两步法首先通过文本匹配缩小候选集，然后根据向量嵌入对这些候选项重新排序。
