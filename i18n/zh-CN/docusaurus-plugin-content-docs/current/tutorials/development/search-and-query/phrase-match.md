...
然而，在实际场景中，严格的短语匹配可能过于死板。您可能希望匹配类似 *"…machine learning models widely adopted in robotics…"* 这样的文本。这里，相同的关键词存在，但并非相邻或按原始顺序排列。为处理这种情况，短语匹配支持 `slop` 参数，该参数引入了灵活性。`slop` 值定义了短语中各词项之间允许的位置偏移数量。例如，当 `slop` 值为 1 时，对 **"machine learning"** 的查询可以匹配类似 *"...machine deep learning..."* 这样的文本，其中一个词（**"deep"**）分隔了原始词项。
...
1. **短语匹配**：执行短语查询时，Zilliz Cloud 会在倒排索引中查找每个词元，并检查它们的位置，以确定它们是否按正确的顺序和邻近度出现。`slop` 参数控制匹配词元之间允许的最大位置数：
    ...
    - **slop = 2** 允许匹配的词元之间最多有两个位置的灵活性或重排。
        ...
        - 这允许反转顺序（**"learning machine"**）或词元之间存在小间隙。
...
在这里，我们允许有 1 的容差。该过滤器应用于包含短语 **"learning machine"** 的文档，并具有一定的灵活性。
...
此示例允许有 2 的容差，这意味着在 **"machine"** 和 **"learning"** 这两个词之间最多允许有两个额外内容（或位置变换的词）。
...
在这个例子中，slop 为 3 提供了更大的灵活性。过滤器搜索 **"machine learning"**，允许单词之间最多有三个词元位置。
...
- 在 Schema 中定义 Analyzer 后，其设置将对该 Collection 永久生效。如果您认为不同的 Analyzer 更适合您的需求，可以考虑删除现有 Collection，并使用所需的 Analyzer 配置创建一个新的 Collection。

- 短语匹配性能取决于文本的分词方式。在将 Analyzer 应用于整个 Collection 之前，请使用 `run_analyzer` 方法查看分词输出。有关更多信息，请参阅 [Analyzer 概述](./analyzer-overview)。

- `filter` 中的转义规则：

    - 表达式中用双引号或单引号括起来的字符会被解释为字符串常量。如果字符串常量包含转义字符，则转义字符必须用转义序列表示。例如，使用 `\\` 表示 `\`，使用 `\\t` 表示制表符 `\t`，使用 `\\n` 表示换行符。
