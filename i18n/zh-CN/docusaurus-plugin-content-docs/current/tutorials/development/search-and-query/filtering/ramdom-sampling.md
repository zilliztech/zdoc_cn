...
notebook: FALSE
description: "处理大规模数据集时，你通常无需处理所有数据就能获得见解或测试过滤逻辑。随机采样提供了一种解决方案，它允许你处理具有统计代表性的数据子集，从而显著减少查询时间和资源消耗。 | Cloud"
type: origin
...
处理大规模数据集时，你通常无需处理所有数据就能获得见解或测试过滤逻辑。随机采样提供了一种解决方案，它允许你处理具有统计代表性的数据子集，从而显著减少查询时间和资源消耗。

随机采样在 segment 层面进行操作，确保在保持样本随机性的同时，在整个 collection 的数据分布中实现高效性能。

**主要用例：**

- **数据探索**：以最少的资源使用快速预览 collection 结构和内容
...
- `sampling_factor`：取值范围为 (0, 1) 的采样因子，不包含边界值。例如，`RANDOM_SAMPLE(0.001)` 大约会选择 0.1% 的结果。
