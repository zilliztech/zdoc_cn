...
所有 Geometry 操作符都通过接收两个几何参数来运行：collection 中定义的 `GEOMETRY` 字段名称，以及以[众所周知的文本](https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry)（WKT）格式表示的目标几何对象。
...
你有一组预定义的兴趣点（例如地标），并且你想通过其精确坐标快速找到特定地标。
