...
Zilliz Cloud 提供了一套丰富的基本操作符，可帮助您高效地过滤和查询数据。通过这些操作符，您可以根据标量字段、数字计算、逻辑条件等细化搜索条件。了解如何使用这些操作符对于构建精确查询并最大限度提升搜索效率至关重要。
...
逻辑操作符通常用来连接多个过滤条件，以实现复杂的过滤条件，包括 `AND`、`OR` 和 `NOT`。
...
如果 JSON 字段值中某些元素（例如，某些键值）为 `null`，该字段的值仍为非空。例如，某 Entity 的 JSON 字段的取值为 `\{"metadata": \{"category": None, "price": 99.99}}`。虽然 `category` 的值为 `None`，但该 JSON 字段的取值不会被当作 `null`。
...
Zilliz Cloud 提供了一系列基本操作符，可让您灵活地过滤和查询数据。通过结合比较、范围、算术和逻辑操作符，您可以创建功能强大的过滤表达式，以缩小搜索结果的范围并高效检索所需数据。
