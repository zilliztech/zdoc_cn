...
您无需将整个 75 个词的长文档发送给 LLM，只需发送 16 个词的答案——并附带显示其与查询相关程度的置信度分数。

<Admonition type="info" icon="📘" title="说明">

Semantic Highlighter 功能依赖托管在 Zilliz Cloud 的 Highlight Model。有关更多信息，请参考[托管模型](./undefined#supported-models)。
...
![ELI1bkji4oOiJGxAdi9cfFb6nXf](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/eli1bkji4ooijgxadi9cffb6nxf.png "ELI1bkji4oOiJGxAdi9cfFb6nXf")
...
本文档中的所有示例都使用此方法。有关更多信息，请参阅 [Model-based Embedding Function](./undefined)。
...
高亮阶段由部署在 Zilliz Cloud 上的托管 [Highlight Model](./undefined) 驱动。该模型处理检索到的文档，识别与查询语义相关的文本片段：
...
| **阈值** | **行为** | **用例** |
| --- | --- | --- |
| 未设置 | 默认使用 0.5 作为阈值。中等和高置信度。 | 更广泛的覆盖范围和中等精度 |
| 0.8 | 高置信度 | 以精度为重点的应用 |
...
- **Highlight 模型部署**
    ...
    有关可用的 Highlight 模型和部署说明，请参阅[托管模型](./undefined)。
...
- **用于语义搜索的 Embedding 模型**
    ...
    **选项 1：基于模型的 Embedding Function（推荐）**
    ...
    - **托管 Embedding 模型**：通过 Zilliz Cloud 部署托管的 embedding 模型，并获取其 `model_deployment_id`。有关可用模型和部署说明，请参阅[托管模型](./undefined)。
    ...
    **选项 2：外部 Embedding 模型**
    ...
    为简化起见，本文档中的所有代码示例都使用基于模型的 Embedding Function 方法（**选项 1**）。
...
此示例展示如何向搜索查询添加 Semantic Highlighter。Semantic Highlighter 会识别与查询语义相关的文本片段，并用指定的标签高亮它们。

<Admonition type="info" icon="📘" title="说明">

将以下代码中的 `YOUR_MODEL_ID` 替换为您托管 Highlight 模型的部署 ID。
...
- 文档 1（ID: 1）获得较高的高亮置信度分数（0.8026），因为它直接回答了关于 AI 何时成立的查询。

- 文档 2（ID: 2）具有中等高亮置信度分数（0.7810），因为它讨论了 AI 历史，但没有具体提到“成立”。

<Admonition type="info" icon="📘" title="说明">

搜索分数（`distance`）反映语义搜索的向量相似度，而高亮置信度（`scores`）反映文本回答特定查询的程度。
