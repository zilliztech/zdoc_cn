...
- [在 Partition 中进行 ANN Search](./single-vector-search#ANN-search-in-partition)
...
本节将演示如何在使用最简建表方式创建的 Collection 中进行单路查询。示例代码中的 Search 请求携带了一个查询向量，要求使用内积（IP）算法计算查询向量和目标向量间的相似度，并返回最相近的 3 个向量。
...
返回的结果将按相似度进行排序，与查询向量最相似的结果排在前面。度量值的大小根据相似度类型的不同呈现出不同的特点。下表展示了使用不同的相似度类型时，其度量值的特点。
...
您也可以在 Search 请求中携带多个查询向量，Zilliz Cloud 将分别针对这些查询向量执行 ANN Search，并返回多组查询结果。
...
## 在 Partition 中进行 ANN Search\{#ANN-search-in-partition}
...
|  | 单页返回 Entity 数量（Limit） | 已返回 Entity 总数量（Offset） |
| --- | --- | --- |
| 第 1 次 | 100 | 0 |
| 第 2 次 | 100 | 100 |
| 第 3 次 | 100 | 200 |
