---
title: "词法 Highlighter | Cloud"
slug: /text-highlighter
sidebar_label: "词法 Highlighter"
beta: FALSE
added_since: FALSE
last_modified: FALSE
...
- **高亮哪些词项**
    ...
    你可以选择高亮词项的来源。例如，高亮 BM25 全文搜索中使用的搜索词，或在基于文本的过滤表达式（如 TEXT_MATCH 条件）中指定的查询词。
...
## BM25 全文搜索中的搜索词高亮\{#search-term-highlighting-in-bm25-full-text-search}

在执行 BM25 全文搜索时，你可以对返回结果中的搜索词进行高亮显示，以帮助解释文档为何与查询匹配。有关 BM25 全文搜索的更多信息，请参阅[全文搜索](./full-text-search)。

在该场景中，被高亮的词项直接来源于 BM25 全文搜索中使用的搜索词。Highlighter 会使用这些词项，对最终结果中的匹配文本进行标注。
...
要在 BM25 全文搜索中高亮搜索词，需要创建一个 `LexicalHighlighter`，并为 BM25 全文搜索启用搜索词高亮：
...
- `highlight_search_text=True` 表示让 Zilliz Cloud 使用 BM25 全文搜索中的搜索词作为高亮词项的来源。
...
在使用 Highlighter 之前，请确保你的 collection 已正确配置。

下面的示例将创建一个同时支持 **BM25 全文搜索** 和 **TEXT_MATCH 查询** 的 collection，并向其中插入示例文档。
...
### 示例 1：在 BM25 全文搜索中高亮搜索词\{#example-1-highlight-search-terms-in-bm25-full-text-search}

本示例演示了如何在 BM25 全文搜索中对搜索词进行高亮显示。

- BM25 全文搜索使用 `"test"` 作为搜索词
...
在 BM25 全文搜索中使用多个查询时，每个查询的结果都会**独立进行高亮**。第一个查询返回的结果只包含其搜索词的高亮，第二个查询返回的结果只包含其搜索词的高亮，依此类推。所有查询共享同一套 Highlighter 配置，但会分别独立应用。
