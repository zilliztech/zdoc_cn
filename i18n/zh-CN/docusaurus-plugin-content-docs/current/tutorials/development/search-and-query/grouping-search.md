...
notebook: FALSE
description: "如果搜索结果中所有 Entity 在某个标量字段上的取值都相同时，搜索结果可能并不能真实反映与查询向量相似的所有向量在向量空间中的分布情况。为了提升召回结果的多样性，可以考虑使用 Grouping Search。本节将介绍如何使用 Grouping Search 以及相关注意事项。 | Cloud"
type: origin
...
如果搜索结果中所有 Entity 在某个标量字段上的取值都相同时，搜索结果可能并不能真实反映与查询向量相似的所有向量在向量空间中的分布情况。为了提升召回结果的多样性，可以考虑使用 Grouping Search。本节将介绍如何使用 Grouping Search 以及相关注意事项。
...
在 Search 请求中，将 `group_by_field` 和 `output_fields` 都设置为 `docId`。收到请求后，Zilliz Cloud 会根据指定的字段对搜索结果分组，并返回每组结果中与查询向量最相似的一个 Entity，同时在每个返回的 Entity 中携带该 Entity 在 `docId` 字段上的取值。
