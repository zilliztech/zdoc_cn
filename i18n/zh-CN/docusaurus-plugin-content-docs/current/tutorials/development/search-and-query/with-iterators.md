...
notebook: FALSE
description: "ANN Search 单次召回的实体数量存在上限，仅使用基本 ANN Search 可能无法满足大规模召回的需求。对于 topK 大于 16,384 的 ANN Search 请求，可以考虑使用 Search Iterator。本节将介绍如何使用 Search Iterator 以及相关注意事项。 | Cloud"
type: origin
...
ANN Search 单次召回的实体数量存在上限，仅使用基本 ANN Search 可能无法满足大规模召回的需求。对于 topK 大于 16,384 的 ANN Search 请求，可以考虑使用 Search Iterator。本节将介绍如何使用 Search Iterator 以及相关注意事项。
...
1. 在创建迭代器时指定单页返回的实体数量和需要返回的实体总数量。
...
创建迭代器后，可以参考如下示例代码循环调用 `next()` 方法获取搜索结果。
