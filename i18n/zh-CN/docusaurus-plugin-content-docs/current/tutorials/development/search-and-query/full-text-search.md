...
设置好 Collection 和索引后，即可插入文本数据。你只需提供原始文本，之前定义的内置函数会自动为每条文本生成对应的稀疏向量。
...
| `params.level` | 使用简化的调优方式控制搜索精度。更多详情，请参考[召回调优](./tune-recall-rate) |
| `data` | 原始查询文本。Zilliz Cloud 使用 BM25 函数自动将查询请求中的文本转换为对应的稀疏向量。因此，请勿提供预生成的向量。 |
| `anns_field` | 用于存储 Milvus 在内部自动生成的稀疏向量的向量字段名称。 |
