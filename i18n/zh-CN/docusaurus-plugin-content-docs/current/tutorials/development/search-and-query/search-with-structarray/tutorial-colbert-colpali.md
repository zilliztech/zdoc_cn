...
在一个简化示例中，查询包含两个 token，即 `machine` 和 `learning`；窗口中的文档包含四个 token：`neural`、`network`、`python` 和 `tutorial`。这些 token 被向量化后，每个查询 token 的 Vector Embedding 会与文档中的 Vector Embedding 比较，得到一组相似度评分。随后，对每组评分取最高值并求和，得到最终评分。确定文档最终评分的过程称为最大相似度（**MAX_SIM**）。关于最大相似度的详细信息，请参见 Maximum similarity。
