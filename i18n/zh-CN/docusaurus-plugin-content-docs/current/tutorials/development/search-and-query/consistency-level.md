...
Zilliz Cloud 是一个存储与计算分离的系统。其中，DataNodes 负责数据持久化，并最终将数据写入 MinIO/S3 之类的分布式对象存储中。QueryNodes 负责 Search 之类的计算任务。这些任务会涉及批式数据和流式数据的处理。简单来说，批式数据可以理解为已经写入对象存储的数据，而流式数据可以理解为尚未写入对象存储的数据。由于网络时延，QueryNodes 往往并不会持有最新的流式数据。在没有其他保障的情况下，直接在流式数据上执行 Search 会遗漏许多尚未写入的数据，从而影响搜索结果的准确性。

![URlRwSlh9hGEBPbbUqAc3ObKn4f](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/URlRwSlh9hGEBPbbUqAc3ObKn4f.png)

如上图所示，QueryNodes 在收到 Search 请求后，可以同时获得流式数据和批式数据。但是，QueryNodes 获取到的流式数据可能会因为网络时延而有所缺失。

为了解决这个问题，Zilliz Cloud 会为数据队列中的每条记录都打上时间戳，并不间断地向数据队列中插入同步时间戳。每当收到同步时间戳时，QueryNodes 就会将其设置为 Service Time，这意味着 QueryNodes 能够看到该 Service Time 之前的所有数据。基于 ServiceTime，Zilliz Cloud 就能够根据不同用户对一致性以及可用性的需求提供保证时间戳（GuaranteeTs）。用户可以通过在 Search 请求中指定 GuaranteeTs，告知 QueryNodes 必须将指定时间点之前的数据纳入搜索范围。
...
如上图所示，如果 GuaranteeTs 小于 ServiceTime，意味着指定时间点之前的数据已经全部落盘，QueryNodes 可以立即执行 Search 操作。当 GuaranteeTs 大于 ServiceTime 时，QueryNodes 必须等待，直到 ServiceTime 大于 GuaranteeTs 后才能执行 Search 操作。

用户需要在查询准确性和查询时延之间做出取舍。如果用户对一致性有较高要求，而对查询时延不敏感，可以将 GuaranteeTs 设置为尽可能大的值；如果用户希望尽快得到搜索结果，而对查询准确性有较高的容忍度，那么可以将 GuaranteeTs 设置为较小的值。
...
- **强一致性（Strong Consistency）**
    ...
    GuaranteeTs 设为系统最新时间戳，QueryNodes 需要等待 ServiceTime 推进到当前最新时间戳后才能执行该 Search 请求；

- **最终一致性（Eventual Consistency）**

    GuaranteeTs 设为一个特别小的值（例如设为 1），跳过一致性检查，立即在当前已有数据上执行 Search 查询；

- **有界一致性（Bounded Staleness）**

    GuaranteeTs 是一个比系统最新时间稍早的时间（Graceful Time），在可容忍范围内可以立即执行查询；
...
### 在创建 Collection 时指定一致性水平\{#set-consistency-level-upon-creating-collection}
