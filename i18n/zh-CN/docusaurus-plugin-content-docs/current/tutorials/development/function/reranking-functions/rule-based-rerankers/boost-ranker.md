...
当搜索请求包含 Boost Ranker 函数时，Zilliz Cloud 会使用该函数中的可选过滤条件，在搜索结果候选集中查找匹配项，并通过应用指定权重来提升这些匹配项的得分，从而在最终结果中提升或降低匹配 Entity 的排名。

## 何时使用 Boost Ranker\{#when-to-use-boost-ranker}

与依赖交叉编码器模型或融合算法的其他 Ranker 不同，Boost Ranker 会将可选的元数据驱动规则直接注入排序过程，因此更适用于以下场景。
...
<table>
   ...
   <tr>
     ...
     <td><ul><li><p>在电子商务搜索结果中突出展示优质产品</p></li><li><p>通过较高的用户参与度指标（如浏览量、点赞数和分享数）提高内容可见性</p></li><li><p>在时效性搜索应用中优先展示近期内容</p></li><li><p>优先展示来自已验证或可信来源的内容</p></li><li><p>提升与精确短语或高相关性关键词匹配的结果</p></li></ul></td>
     <td rowspan="2"><p>无需重建索引或修改向量嵌入模型（这些操作可能很耗时），你可以通过实时应用可选的元数据过滤器，立即提升或降低搜索结果中特定项目的排名。这种机制实现了灵活、动态的搜索排名，能够轻松适应不断变化的业务需求。</p></td>
   </tr>
   <tr>
     <td><p>战略性内容降权</p></td>
     <td><ul><li><p>在不完全移除低库存商品的情况下降低其突出程度</p></li><li><p>在不进行审查的情况下降低包含潜在冒犯性词汇内容的排名</p></li><li><p>在保留旧版归档以供技术搜索访问的同时对其进行降级处理</p></li><li><p>在市场搜索中巧妙降低竞品的可见度</p></li><li><p>降低存在低质量信号（如格式问题、篇幅较短等）的内容的相关性</p></li></ul></td>
   </tr>
</table>
...
当你插入数据时，Zilliz Cloud 会将它们分配到不同的 Segment 中。当你执行搜索时，每个 Segment 都会返回一组指定数量的候选结果，Zilliz Cloud 会对所有 Segment 返回的候选结果进行排序，并将其归约为指定数量的最终结果。在接收到携带 Boost Ranker 的搜索请求时，Zilliz Cloud 会将 Boost Ranker 应用于每个 Segment 返回的候选搜索结果，以防止可能的精度损失并提高召回率。
...
以下示例展示了在单向量搜索中使用 Boost Ranker 的情况，该搜索需要返回前五个最相关的 Entity，并为文档类型为摘要的 Entity 得分添加权重。

1. **分阶段收集搜索结果候选对象。**

    下表假设 Milvus 将 Entity 分配到两个 Segment（**0001** 和 **0002**）中，每个 Segment 返回五个候选 Entity。

    | ID | Doc Type | 得分 | 排名 | Segment |
    | --- | --- | --- | --- | --- |
...
1. **应用在 Boost Ranker 中指定的权重**（`权重=0.5`）。
    ...
    上一步中所有已识别的 Entity 都将乘以 Boost Ranker 中指定的权重，从而导致其排名发生变化。

    | ID | Doc Type | 得分 | 加权分数<br/>(=得分×权重) | 排名 | Segment |
    | --- | --- | --- | --- | --- | --- |
    ...
    权重必须是你选择的一个浮点数。在上述示例这类场景中，分数越小表示相关性越高，因此使用小于 **1** 的权重。否则，使用大于 **1** 的权重。

    </Admonition>

1. **根据加权分数汇总所有 Segment 的候选对象，以确定最终结果。**

    | ID | Doc Type | 得分 | 加权得分 | 排名 | Segment |
    | --- | --- | --- | --- | --- | --- |
    | **117** | **Abstract** | **0.344** | **0.172** | **1** | **0001** |
...
在将 Boost Ranker 作为搜索请求的重排器传递之前，你应按如下方式将 Boost Ranker 正确定义为重排函数：
...
<table>
   ...
   <tr>
     ...
     <td><p>是</p></td>
     <td><p>要调用的函数类型；使用 <code>RE</code> 指定重排策略</p></td>
   ...
   <tr>
     ...
     <td><p>是</p></td>
     <td><p>指定将与原始搜索结果中任何匹配 Entity 的得分相乘的权重。</p><p>该值应为浮点数。</p><ul><li><p>要强调匹配 Entity 的重要性，请将其设置为能提高分数的值。</p></li><li><p>要降低匹配 Entity 的排名，请将此参数设置为能降低其得分的值。</p></li></ul></td>
     <td><p><code>1</code></p></td>
   </tr>
   <tr>
     <td><p><code>params.filter</code></p></td>
     <td><p>否</p></td>
     <td><p>指定用于在搜索结果 Entity 中匹配 Entity 的过滤表达式。它可以是<a href="./filtering-overview">过滤表达式概览</a>中提到的任何有效基本过滤表达式。</p><p><strong>注意</strong>：仅使用基本运算符，例如 <code>==</code>、<code>&gt;</code> 或 <code>&lt;</code>。使用高级运算符（例如 <code>text_match</code> 或 <code>phrase_match</code>）会降低搜索性能。</p></td>
     <td><p><code>"doctype == 'abstract'"</code></p></td>
   </tr>
   <tr>
     <td><p><code>params.random_score</code></p></td>
     <td><p>否</p></td>
     <td><p>指定一个随机函数，该函数会随机生成一个介于 <code>0</code> 和 <code>1</code> 之间的值。它有以下两个可选参数：</p><ul><li><p><code>seed</code>（数字）指定用于启动伪随机数生成器（PRNG）的初始值。</p></li><li><p><code>field</code>（字符串）指定一个字段的名称，该字段的值将用作生成随机数的随机因子。具有唯一值的字段就足够了。</p><p>建议你同时设置 <code>seed</code> 和 <code>field</code>，以通过使用相同的 seed 和字段值来确保多次生成之间的一致性。</p></li></ul></td>
...
一旦 Boost Ranker 函数准备就绪，你就可以在搜索请求中引用它。以下示例假设你已经创建了一个包含以下字段的集合：**id**、**vector** 和 **doctype**。
...
你可以在单次搜索中组合多个 Boost Ranker，以影响搜索结果。为此，你可以创建多个 Boost Ranker，并在 **FunctionScore** 实例中引用它们，然后将 **FunctionScore** 实例用作搜索请求中的 Ranker。

以下示例展示了如何通过应用介于 **0.8** 和 **1.2** 之间的权重来修改所有已识别 Entity 的分数。
...
具体来说，其中有两个 Boost Ranker：一个对所有找到的 Entity 应用固定权重，另一个为它们分配随机权重。然后，我们在 **FunctionScore** 中引用这两个 Boost Ranker，该 Ranker 还定义了权重如何影响找到的 Entity 的得分。
...
<table>
   ...
   <tr>
     ...
     <td><p>否</p></td>
     <td><p>指定指定权重如何影响任何匹配 Entity 的得分。</p><p>可能的值为：</p><ul><li><p><code>Multiply</code></p><p>表示加权值等于匹配 Entity 的原始得分乘以指定权重。</p><p>这是默认值。</p></li><li><p><code>Sum</code></p><p>表示加权值等于匹配 Entity 的原始分数与指定权重之和。</p></li></ul></td>
     <td><p><code>"Multiply"</code></p></td>
   </tr>
   <tr>
     <td><p><code>params.function_mode</code></p></td>
     <td><p>否</p></td>
     <td><p>指定如何处理来自各个 Boost Ranker 的加权值。</p><p>可能的值为：</p><ul><li><p><code>Multiply</code></p><p>表示匹配 Entity 的最终得分等于所有 Boost Ranker 的加权值的乘积。</p><p>这是默认值。</p></li><li><p><code>Sum</code></p><p>表示匹配 Entity 的最终得分等于所有 Boost Ranker 的加权值之和。</p></li></ul></td>
     <td><p><code>"Multiply"</code></p></td>
   </tr>
</table>
