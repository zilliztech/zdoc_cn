...
notebook: FALSE
description: "硅基流动 Ranker 利用硅基流动的综合重排序模型，通过语义重排序来提升搜索相关性。它提供灵活的文档分块功能，并支持来自不同供应商的多种专业重排序模型。 | Cloud"
type: origin
...
硅基流动 Ranker 利用[硅基流动](https://www.siliconflow.com/)的综合重排序模型，通过语义重排序来提升搜索相关性。它提供灵活的文档分块功能，并支持来自不同供应商的多种专业重排序模型。

硅基流动 Ranker 对以下应用尤其有价值：

- 需要具备可配置重叠部分的高级文档分块，以处理长文档

- 需要访问多种重排序模型，包括 BAAI/bge reranker 系列和其他专业模型

- 需要基于灵活分块的评分机制，其中得分最高的分块代表文档得分

- 需要具有成本效益的重排序，并支持标准和专业模型变体
...
- **与硅基流动集成并获取集成 ID**
    ...
    您必须在 Zilliz Cloud 中创建与硅基流动的模型供应商集成，并获取集成 ID。详见[模型供应商](./integrate-with-model-providers)。
