...
notebook: FALSE
description: "RRF（Reciprocal Rank Fusion，互反排名融合）Ranker 是 Zilliz Cloud 混合搜索中的一种重排序策略，它会根据多个向量搜索路径中的排名位置而非原始相似度得分来平衡结果。就像体育锦标赛会关注选手的名次而非个人统计数据一样，RRF Ranker 会根据每个项目在不同搜索路径中的排名高低合并搜索结果，从而生成公平且均衡的最终排名。 | Cloud"
type: origin
...
RRF（Reciprocal Rank Fusion，互反排名融合）Ranker 是 Zilliz Cloud 混合搜索中的一种重排序策略，它会根据多个向量搜索路径中的排名位置而非原始相似度得分来平衡结果。就像体育锦标赛会关注选手的名次而非个人统计数据一样，RRF Ranker 会根据每个项目在不同搜索路径中的排名高低合并搜索结果，从而生成公平且均衡的最终排名。

## 何时使用 RRF Ranker\{#when-to-use-rrf-ranker}

RRF Ranker 专为混合搜索场景而设计。在这些场景中，你可以平衡来自多个向量搜索路径的结果，而无需分配明确的重要性权重。它在以下方面特别有效：

| 用例 | 示例 | RRF Ranker 为何效果良好 |
| --- | --- | --- |
| 同等重要性的多模态搜索 | 图像文本搜索，两种模态同等重要 | 平衡结果，无需进行任意权重分配 |
| 集成向量搜索 | 结合不同嵌入模型的结果 | 以公平方式合并排名，不偏向任何特定模型的得分分布 |
| 跨语言搜索 | 跨多种语言查找文档 | 公平地对结果进行排名，而不受特定语言的嵌入特征影响 |
| 专家建议 | 整合多个专家系统的建议 | 当不同系统使用不可比较的评分方法时，创建共识排名 |

如果你的混合搜索应用需要在不分配明确权重的情况下公平地平衡多个搜索路径，RRF Ranker 是理想选择。

## RRF Ranker 的工作机制\{#mechanism-of-rrf-ranker}

RRF Ranker 策略的主要工作流程如下：

1. **收集搜索排名**：收集各个向量搜索路径结果的排名（rank_1、rank_2）。

1. **合并排名**：根据公式转换每条路径中的排名（rank_rrf_1、rank_rrf_2）。

    计算公式涉及 *N*，表示检索次数。*ranki*(*d*) 是由*第 i* 个检索器生成的文档 *d* 的排名位置。*k* 是一个平滑参数，通常设置为 60。

1. **聚合排名**：根据组合后的排名对搜索结果进行重新排序，以生成最终结果。
...
此示例展示了在稀疏-密集向量上执行的混合搜索（topK=5），并说明 RRF Ranker 策略如何对两次近似最近邻（ANN）搜索的结果进行重排序。
...
- 使用 RRF 对两组搜索结果的排名进行重排。假设平滑参数 `k` 设置为 60。
...
使用 RRF Ranker 策略时，需要配置参数 `k`。它是一个平滑参数，可以有效改变全文搜索与向量搜索的相对权重。该参数的默认值为 60，可在 (0, 16384) 范围内调整。取值应为浮点数，建议值在 [10, 100] 之间。虽然 `k=60` 是常见选择，但最优 `k` 值可能因具体应用和数据集而异。建议根据具体用例测试和调整此参数，以获得最佳性能。

### 创建 RRF Ranker\{#rrf-ranker}

为你的 collection 设置多个向量字段后，使用适当的平滑参数创建一个 RRF Ranker：
...
| `params.k` | 否 | 平滑参数，用于控制文档排名的影响；较高的 `k` 会降低对高排名的敏感度。范围：(0, 16384)；默认值：`60`。<br/>详情请参考 [RRF Ranker 的机制](./reranking-rrf#mechanism-of-rrf-ranker)。 | `100` |
