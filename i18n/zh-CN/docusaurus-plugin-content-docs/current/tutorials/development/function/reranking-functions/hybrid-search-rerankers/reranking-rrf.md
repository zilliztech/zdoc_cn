...
notebook: FALSE
description: "RRF（Reciprocal Rank Fusion，互反排名融合）Ranker 是 Zilliz Cloud 混合搜索中的一种重排序策略，它根据多个向量搜索路径中的排名位置而非原始相似度得分来平衡结果。就像体育锦标赛会考虑选手的名次而非个人统计数据一样，RRF Ranker 会根据每个项目在不同搜索路径中的排名高低来合并搜索结果，从而生成公平且均衡的最终排名。 | Cloud"
type: origin
...
RRF（Reciprocal Rank Fusion，互反排名融合）Ranker 是 Zilliz Cloud 混合搜索中的一种重排序策略，它根据多个向量搜索路径中的排名位置而非原始相似度得分来平衡结果。就像体育锦标赛会考虑选手的名次而非个人统计数据一样，RRF Ranker 会根据每个项目在不同搜索路径中的排名高低来合并搜索结果，从而生成公平且均衡的最终排名。

## 何时使用 RRF Ranker\{#when-to-use-rrf-ranker}

RRF Ranker 专为混合搜索场景而设计。在这些场景中，你可以平衡来自多个向量搜索路径的结果，而无需分配明确的重要性权重。它在以下场景中特别有效：

| 用例 | 示例 | 为什么 RRF Ranker 效果良好 |
| --- | --- | --- |
| 同等重要性的多模态搜索 | 图像-文本搜索，两种模态同等重要 | 平衡结果，无需进行任意权重分配 |
| 集成向量搜索 | 结合不同 embedding 模型的结果 | 以民主方式合并排名，不偏向任何特定模型的得分分布 |
| 跨语言搜索 | 跨多种语言查找文档 | 公平地对结果进行排名，而不受特定语言 embedding 特征的影响 |
| 专家推荐 | 整合多个专家系统的推荐 | 当不同系统使用不可比较的评分方法时，创建共识排名 |
...
1. **收集搜索排名**：收集各个向量搜索路径结果的排名（rank_1、rank_2）。

1. **合并排名**：根据公式转换每条路径的排名（rank_rrf_1、rank_rrf_2）。

    该计算公式涉及 *N*，表示检索次数。*ranki*(*d*) 是由*第 i* 个检索器生成的文档 *d* 的排名位置。*k* 是平滑参数，通常设置为 60。
...
- 使用 RRF 对两组搜索结果的排名进行重新排列。假设平滑参数 `k` 设置为 60。
...
使用 RRF Ranker 策略时，需要配置参数 `k`。它是一个平滑参数，可以有效改变全文搜索与向量搜索的相对权重。该参数的默认值为 60，可在 (0, 16384) 范围内调整。取值应为浮点数，建议值在 [10, 100] 之间。虽然 `k=60` 是常见选择，但最优 `k` 值可能因具体应用和数据集而异。我们建议根据具体用例测试和调整此参数，以获得最佳性能。

### 创建 RRF Ranker\{#rrf-ranker}

在你的 collection 设置了多个向量字段后，使用合适的平滑参数创建 RRF Ranker：
...
| `input_field_names` | 是 | 要应用该函数的向量字段列表（对于 RRF Ranker 必须为空） | [] |
| `function_type` | 是 | 要调用的函数类型；使用 `RERANK` 指定重排序策略 | `FunctionType.RERANK` |
| `params.reranker` | 是 | 指定要使用的重排序方法。<br/>必须设置为 `rrf` 才能使用 RRF Ranker。 | `"rrf"` |
| `params.k` | 否 | 平滑参数，用于控制文档排名的影响；较高的 `k` 会降低对靠前排名的敏感度。范围：(0, 16384)；默认值：`60`。<br/>详情请参阅 [RRF Ranker 工作机制](./reranking-rrf#mechanism-of-rrf-ranker)。 | `100` |
