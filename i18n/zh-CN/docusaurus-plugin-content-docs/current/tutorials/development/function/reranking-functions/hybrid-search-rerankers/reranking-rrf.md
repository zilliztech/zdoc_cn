...
notebook: FALSE
description: "RRF（互反排名融合）Ranker 是 Zilliz Cloud 混合搜索的一种重排序策略，它根据多个向量搜索路径中的排名位置而非原始相似度得分来平衡结果。就像体育锦标赛会考虑选手的排名而非个人统计数据一样，RRF Ranker 会根据每个项目在不同搜索路径中的排名高低来合并搜索结果，从而生成公平且平衡的最终排名。 | Cloud"
type: origin
...
RRF（互反排名融合）Ranker 是 Zilliz Cloud 混合搜索的一种重排序策略，它根据多个向量搜索路径中的排名位置而非原始相似度得分来平衡结果。就像体育锦标赛会考虑选手的排名而非个人统计数据一样，RRF Ranker 会根据每个项目在不同搜索路径中的排名高低来合并搜索结果，从而生成公平且平衡的最终排名。
...
如果你的混合搜索应用程序需要在不分配明确权重的情况下民主地平衡多个搜索路径，RRF Ranker 是理想选择。
...
- 使用 RRF 重新排列两组搜索结果的排名。假设平滑参数 `k` 设置为 60。
...
| `function_type` | 是 | 要调用的函数类型；使用 `RERANK` 来指定重排序策略 | `FunctionType.RERANK` |
