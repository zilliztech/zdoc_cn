...
所有时间参数（`origin`、`offset`、`scale`）必须使用与 Collection 数据相同的单位。如果您的 Collection 以不同的单位（毫秒、微秒）存储时间戳，请相应地调整所有参数。
...
在您的 Collection 设置了一个数字字段（在本示例中，`event_date` 表示从现在起的秒数）之后，创建一个线性衰减排序器：
