...
现在，让我们创建三个不同的 Decay Ranker，每个 Ranker 都有不同的参数，以突出它们的差异：
...
- `decay`：偏移量 + 比例下的衰减因子（例如，0.5 表示相关性减半）

请注意，我们已使用不同的参数配置了指数 Ranker，以展示如何针对不同的行为调整这些函数。
...
让我们详细分析最终得分是如何通过将原始相关性与衰减因子相结合来计算的：
...
通过理解衰减函数背后的数学原理，并尝试不同的参数，您可以微调搜索系统，从而为您的特定用例在相关性和新鲜度之间实现最佳平衡。
