...
1. **文本分析**：当 `token_level` 为 `"word"` 时，文本会由 Analyzer 概述处理；当 `token_level` 为 `"char"` 时，文本会直接使用原始字符。词级别分词会使用输入字段上配置的 Analyzer 将文本切分为词元。例如，`"milvus is vector db"` 会被切分为 `["milvus", "is", "vector", "db"]`。
