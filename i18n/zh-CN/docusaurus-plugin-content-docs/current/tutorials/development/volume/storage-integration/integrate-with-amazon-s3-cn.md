...
1. 在弹出的对话框中，完成基础设置：
    ...
    - **配置名称**：为此集成设置一个唯一名称（例如：`s3_integration_0820`）。

    - **配置描述**（可选）：为此集成添加描述（例如：`for export backup file`）。
...
1. 将[步骤 2](./integrate-with-amazon-s3-cn#create-s3-bucket) 中 Zilliz Cloud 提供的 JSON 策略文档粘贴到编辑器中，然后点击**下一步**。
    ...
    当您为上述对象存储桶启用了服务端加密时，您还需要为 `kms:GenerateDataKey` 操作添加相应的策略。在此情况下，请使用如下 JSON 策略文件。
...
1. 创建 OSS 存储空间。
    ...
    更多详情，可参考上面的[创建存储桶](./integrate-with-alibaba-cloud-oss#step-2-create-s3-bucket)小节或 [CreateBucket](https://docs.aws.amazon.com/zh_cn/AmazonS3/latest/API/API_CreateBucket.html)。
