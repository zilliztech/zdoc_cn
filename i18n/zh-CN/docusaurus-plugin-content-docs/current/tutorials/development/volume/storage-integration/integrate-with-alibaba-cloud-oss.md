...
1. 在弹出的对话框中，完成基础设置：
    ...
    - **Bucket 权限**：选择 Zilliz Cloud 对您的对象存储空间具有的访问级别。下表说明了这些选项。

        | **权限** | **说明** |
        | --- | --- |
        | 只读 | Zilliz Cloud 只能从存储空间中读取文件。用于支持 External Collections 的 [External volumes](./external-volume)。 |
...
1. 在**创建 Bucket** 面板中，按如下说明配置各项参数。

    1. **Bucket 名称**：为您的 OSS 存储空间设置一个唯一名称。

    1. **地域**：指定存储空间所在的云地域。注意，存储空间所在地域必须和目标 Zilliz Cloud 集群或 Volume 所在的地域相同。有关 Zilliz Cloud 支持的云地域，请参考[云服务提供商和地域](./cloud-providers-and-regions)。
...
1. 在 [RAM 控制台](https://ram.console.aliyun.com/)中，选择**权限管理 > 权限策略**。
...
1. 在**创建权限策略**对话框中，输入权限策略**名称**和**备注**，然后单击**确定**。
...
1. 在 [RAM 控制台](https://ram.console.aliyun.com/)中，选择**角色** > **创建角色**。
...
1. 在弹出的**新增授权**面板中，搜索并选择[步骤 3](./integrate-with-alibaba-cloud-oss) 中创建的权限策略，然后点击**确认新增授权**，最后单击**关闭**。
...
1. 在 [Zilliz Cloud 控制台](https://cloud.zilliz.com.cn/login)中，点击**校验集成**，验证当前配置的存储空间和 RAM 角色是否有效。
...
如果您需要使用 RESTful API 接口导出文件到您集成到 Zilliz Cloud 的对象存储空间中，您可以单击**查看详情**查看该对象存储空间的集成详情并复制该存储空间的集成 ID。
