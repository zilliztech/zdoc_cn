---
title: "Serving 集群中的 Database | Cloud"
slug: /database
sidebar_label: "Serving 集群中的 Database"
beta: FALSE
added_since: FALSE
last_modified: FALSE
...
type: origin
token: XIFowGwHFiNkgbkONbgcfhHTnbe
sidebar_position: 2
...
![QxbhwODEShDTXobgNoYc8tlmnqc](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/QxbhwODEShDTXobgNoYc8tlmnqc.png)
...
仅 Dedicated 集群支持创建 Database。在您创建 Dedicated 集群的同时，Zilliz Cloud 会为您在集群下自动创建一个默认 Database。
...
![create-database-cn](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/create-database-cn.png "create-database-cn")
...
| 属性名称 | 值类型 | 属性描述 |
| --- | --- | --- |
| `database.replica.number` | integer | 指定 Database 的副本数量。 |
| `database.max.collections` | integer | 指定 Database 中可以创建的 Collection 的最大数量。 |
| `database.force.deny.writing` | boolean | 当前 Database 是否强制不可写。 |
| `database.force.deny.reading` | boolean | 当前 Database 是否强制不可读。 |
|  | string |  |
...
![drop-database-cn](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/drop-database-cn.png "drop-database-cn")
