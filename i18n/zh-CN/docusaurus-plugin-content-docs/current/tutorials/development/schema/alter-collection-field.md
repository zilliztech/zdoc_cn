...
如下示例假设您的 Collection 中有一个名为 `varchar` 的字段，并修改该字段的 `max_length` 属性。
...
如下示例假设您的 Collection 中有一个名为 `array` 的字段，并修改该字段的 `max_capacity` 属性。
...
如下示例假设您的 Collection 中有一个名为 `doc_chunk` 的字段，并设置其 `mmap_enabled` 属性。
