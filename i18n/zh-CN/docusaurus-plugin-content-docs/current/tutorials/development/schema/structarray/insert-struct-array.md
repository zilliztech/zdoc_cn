...
Nullable StructArray Field 仅在与 Milvus v3.0.x 兼容的集群中可用。如果你动态向现有 Collection 添加 StructArray Field，则新增 Field 必须是 Nullable，已有 Entity 会在新 Field 的所有 subfield 上返回 `null`。
