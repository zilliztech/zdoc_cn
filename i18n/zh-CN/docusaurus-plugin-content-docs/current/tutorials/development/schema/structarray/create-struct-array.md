...
与 Milvus v3.0.x 兼容的集群支持 Nullable StructArray Field。Nullable StructArray Field 允许 Entity 为整个 StructArray Field 存储 `null`。
...
Nullable StructArray Field 仅在与 Milvus v3.0.x 兼容的集群中可用。对于 Nullable StructArray Field，Entity 可以提供有效的 StructArray value，也可以将整个 Field 设为 `null`。插入有效的 StructArray value 时，所有 subfield 应同时为 null 或同时具有有效值。如果某些 subfield 为 null 而其他 subfield 为有效值，会导致错误。详情请参阅 [StructArray 限制](./struct-array-limits)。

</Admonition>

## 向现有 Collection 添加 StructArray Field \{#add-a-structarray-field-to-an-existing-collection}

与 Milvus v3.0.x 兼容的集群支持向现有 Collection 添加 StructArray Field。新增的 StructArray Field 必须是 Nullable，因为 Collection 中已存在的 Entity 没有这个新 Field 的值。
