---
title: "为 StructArray Field 创建 Index | Cloud"
...
notebook: FALSE
description: "在运行 Vector Search 或加速 Scalar Filtering 前，请先在 StructArray subfield 上创建 Index。对于 StructArray Field，Index 目标是 `chunks[emblistvector]`、`chunks[emb]` 或 `chunks[section]` 等 subfield path。 | Cloud"
type: origin
...
| Vector subfield 数据类型 | Index type | Metric type |
| --- | --- | --- |
...
| 搜索模式 | Vector subfield 数据类型 | Index type | Metric type |
| --- | --- | --- | --- |
