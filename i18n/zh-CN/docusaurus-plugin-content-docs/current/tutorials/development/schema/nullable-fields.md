...
notebook: FALSE
description: "Zilliz Cloud 支持 `nullable` 属性。启用 `nullable` 属性后，字段值可以缺失，或显式设置为 `NULL`。该属性在 Schema 级别定义，并在数据写入、索引、搜索和查询操作中保持一致。 | Cloud"
type: origin
...
## 什么是 Nullable 属性\{#nullable-attribute}
...
前面的示例重点介绍了向量字段。本节说明 **标量过滤表达式** 中 `NULL` 值的行为。
...
| Nullable | 默认值 | 用户输入 | 结果 |
| --- | --- | --- | --- |
| ✅ | ✅（非 `NULL`） | `NULL` 或省略 | 使用默认值 |
| ✅ | ❌ | `NULL` 或省略 | 存储为 `NULL` |
| ❌ | ✅（非 `NULL`） | `NULL` 或省略 | 使用默认值 |
| ❌ | ❌ | `NULL` 或省略 | 抛出错误 |
| ❌ | ✅（`NULL`） | `NULL` 或省略 | 抛出错误 |
