...
- `FLOAT16_VECTOR`
    ...
    用于存放 16 位半精度浮点数列表，通常用于内存和带宽受限的深度学习和 GPU 计算场景。
...
- `BINARY_VECTOR`
    ...
    用于存放二进制数据，通常为一组由 0 和 1 组成的数字序列。它们在图像处理和信息检索场景下用于表示数据的紧凑特征。
...
通常情况下，您可以将标量字段作为向量字段的元数据添加到 Collection 中，通过向量搜索和标量过滤相结合的方式来提升搜索结果的准确性。Zilliz Cloud 支持多种标量数据类型，包括 **VarChar**、**Boolean**、**Int**、**Float**、**Double**、**Array** 和 **JSON**。
