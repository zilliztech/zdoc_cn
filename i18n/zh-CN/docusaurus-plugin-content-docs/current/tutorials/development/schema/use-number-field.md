...
为标量数值字段设置索引参数可以提升查询和搜索效率。在 Zilliz Cloud 集群中，为向量字段创建索引是必选操作。对于包括布尔和数值类型在内的标量字段而言，该操作为可选。

以下示例中，我们为向量字段 `embedding` 和标量字段 `age` 创建了 `AUTOINDEX` 类型的索引，表示 Milvus 会根据数据类型自动创建合适的索引。有关更多信息，请参考 [AUTOINDEX](./autoindex-explained)。
