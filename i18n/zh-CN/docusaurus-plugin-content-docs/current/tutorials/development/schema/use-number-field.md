...
为标量数值字段设置索引参数可以提升查询和搜索效率。在 Zilliz Cloud 集群中，为向量字段创建索引是必选操作。对于包括布尔和数值类型的标量字段而言，该操作为可选。
...
除了简单的基于数值的过滤查询外，您还可以将向量相似性搜索与过滤表达式结合使用。以下示例演示了如何在向量搜索中添加过滤表达式。
...
在此示例中，我们首先定义了一个查询向量，然后添加了过滤表达式 `25 <= age <= 35`。查询结果除了需要与查询向量在语义上相关外，还必须满足过滤表达式的要求。关于过滤查询的更多信息，请参阅[过滤搜索](./filtered-search)。
