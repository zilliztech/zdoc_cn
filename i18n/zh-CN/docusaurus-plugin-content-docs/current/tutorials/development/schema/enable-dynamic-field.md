...
由于 dynamic field 是一个 JSON 字段，您可以使用 JSON 路径语法来索引其中的任意键。这适用于简单的标量值，也适用于复杂的嵌套结构。
...
有关支持的操作符和过滤表达式的完整列表，请参考 [过滤搜索](./filtered-search)。
...
- **加载 collection**
    ...
    参考 [加载和释放](./load-release-collections)

- **使用 JSON 过滤表达式进行搜索或查询**  

    参考 [过滤搜索](./filtered-search) 和 [JSON 操作符](./json-filtering-operators)
