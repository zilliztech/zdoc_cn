...
notebook: FALSE
description: "Zilliz Cloud 中的 English Analyzer 旨在处理英语文本，应用特定语言的规则进行分词和过滤。 | Cloud"
type: origin
...
Zilliz Cloud 中的 English Analyzer 旨在处理英语文本，应用特定语言的规则进行分词和过滤。

## 定义\{#definition}

English Analyzer 使用以下组件：
...
- 过滤器：包括多个过滤器以进行全面的文本处理： 
    ...
    - `stemmer`：将单词还原为其词根形式，以支持更广泛的匹配（例如，“running”变为“run”）。

    - `stop_words`：删除常见的英语停用词，以便重点关注文本中的关键术语。
...
| 参数 | 描述 |
| --- | --- |
| `stop_words` | 包含停用词列表的数组，这些词将在分词过程中被移除。默认为 `_english_`，即内置的常见英语停用词集合。 |
