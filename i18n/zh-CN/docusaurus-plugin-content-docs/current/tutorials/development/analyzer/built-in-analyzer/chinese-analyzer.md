...
notebook: FALSE
description: "中文分析器专门设计用于处理中文文本，提供有效的分段和分词。 | Cloud"
type: origin
...
要将中文分析器应用于字段，只需在 `analyzer_params` 中将 `type` 设置为 `chinese`，并根据需要包含可选参数。  
...
在完成分析器配置后，您可以使用 `run_analyzer` 方法来验证分词效果是否符合预期。
