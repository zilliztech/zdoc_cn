...
为了更好地提升查询和搜索性能，根据您的文本语言选择合适的 Analyzer 十分必要。虽然 `standard` Analyzer 适用于大多数西方语言，但对于中文、日语和韩语这类具有特殊语法结构的语言来说并不是最佳选择。在这种情况下，建议您选择使用 [`chinese`](./chinese-analyzer)，或自定义使用指定分词器（如 lindera 或 icu）和过滤器的 Analyzer。准确分词可以极大保障搜索结果的可靠性。  
