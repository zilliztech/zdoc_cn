...
notebook: FALSE
description: "`lindera` 分词器执行基于词典的形态素分析，专为日语和韩语设计——这两种语言的词汇之间没有空格分隔，且语法标记（助词）直接附着在词汇上。 | Cloud"
type: origin
...
要配置使用 `lindera` 分词器的 analyzer，将 `tokenizer.type` 设置为 `lindera`，通过 `dict_kind` 选择词典，并可选地应用 filter。
...
<table>
   ...
   <tr>
     ...
     <td><p>分词器类型。使用 Lindera 分词器时，该值为 <code>lindera</code>。</p></td>
   </tr>
   <tr>
     <td><p><code>dict_kind</code></p></td>
     <td><p>作为参考词表的词典。取值范围如下：</p><ul><li><p><code>ko-dic</code>：韩语 - 韩语形态素词典 (<a href="https://bitbucket.org/eunjeon/mecab-ko-dic">MeCab Ko-dic</a>)</p></li><li><p><code>ipadic</code>：日语 - 标准形态素词典 (<a href="https://taku910.github.io/mecab/">MeCab IPADIC</a>)</p></li></ul></td>
   </tr>
   <tr>
     <td><p><code>filter</code></p></td>
     <td><p>分词后应用的分词器级别 filter 列表。每个 filter 是一个包含以下字段的对象：</p><ul><li><p><code>kind</code>：filter 类型。支持的值：</p><ul><li><p><code>korean_stop_tags</code>：移除匹配指定韩语词性标签的 token。</p></li><li><p><code>japanese_stop_tags</code>：移除匹配指定日语词性标签的 token。</p></li></ul></li><li><p><code>tags</code>：要过滤的词性标签列表。可用标签取决于 <code>kind</code> 的值：</p><ul><li><p>对于 <code>korean_stop_tags</code>：使用准确的标签代码（如 <code>JKS</code>、<code>JKO</code>、<code>SF</code>）。韩语标签需要精确匹配。基于世宗标注体系的完整标签列表请参见 <a href="https://docs.rs/lindera/latest/src/lindera/token_filter/korean_stop_tags.rs.html">Lindera 韩语 stop tags 源码</a>。</p></li><li><p>对于 <code>japanese_stop_tags</code>：对于 <code>japanese_stop_tags</code>：使用准确的标签代码（如 <code>助詞,格助詞</code>、<code>助詞,係助詞</code>、<code>助動詞</code>）。日语标签需要精确匹配。完整的标签列表（IPADIC）请参见<a href="https://github.com/taku910/mecab/blob/master/mecab-ipadic/pos-id.def">日语词性标签参考</a>。</p></li></ul></li></ul></td>
   </tr>
</table>

定义 `analyzer_params` 后，您可以在定义 Collection Schema 时将其应用到 VARCHAR 类型的字段上。Zilliz Cloud 将根据 analyzer 的设置对该字段的内容进行分词和过滤。更多详情，请参考[使用示例](./analyzer-overview#example-use)。

## 使用示例\{#examples}

完成 analyzer 配置后，您可以使用 `run_analyzer` 方法验证分词效果是否符合预期。
