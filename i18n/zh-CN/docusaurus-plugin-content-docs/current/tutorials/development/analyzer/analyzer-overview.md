...
结果显示了当前 Analyzer 对输入文本进行了处理，丢弃了诸如 `"a"`、`"an"` 和 `"for"` 这样的停用词，仅返回了有明确含义的词元。
...
在验证了 Analyzer 的配置后，您就可以参考如下示例将 Analyzer 添加到目标字段中。
...
在配置 Analyzer 时，建议您参考如下文章了解如何针对您的数据集选择合适的 Analyzer 配置。
