...
`decompounder` 过滤器是 Zilliz Cloud 中的自定义过滤器，可通过在过滤器配置中设置 `"type": "decompounder"` 并将 `word_list` 设置为所需值来指定。
...
| 参数 | 描述 |
| --- | --- |
| `word_list` | 用于拆分复合术语的词组件列表。该字典决定复合词如何被分解为单个术语。 |

`decompounder` 过滤器作用于分词器生成的词项，因此必须与分词器结合使用。有关 Zilliz Cloud 中可用分词器的列表，请参阅[分词器参考](./undefined)。
