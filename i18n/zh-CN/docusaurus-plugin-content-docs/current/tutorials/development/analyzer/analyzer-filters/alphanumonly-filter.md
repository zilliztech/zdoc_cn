...
notebook: FALSE
description: "`alphanumonly` 过滤器删除包含非 ASCII 字符的词项，仅保留字母数字词项。该过滤器在处理仅与基本字母和数字相关的文本时非常有用，可排除任何特殊字符或符号。 | Cloud"
type: origin
...
`alphanumonly` 过滤器删除包含非 ASCII 字符的词项，仅保留字母数字词项。该过滤器在处理仅与基本字母和数字相关的文本时非常有用，可排除任何特殊字符或符号。
...
`alphanumonly` 过滤器作用于分词器生成的词项，因此必须与分词器结合使用。有关 Zilliz Cloud 中可用分词器的列表，请参阅[分词器参考](./undefined)。

定义 `analyzer_params` 后，您可以在定义 Collection Schema 时将其应用于 VARCHAR 字段。这使 Zilliz Cloud 能够使用指定的分析器处理该字段中的文本，从而实现高效的分词和过滤。更多信息，请参阅[使用示例](./analyzer-overview#example-use)。  
