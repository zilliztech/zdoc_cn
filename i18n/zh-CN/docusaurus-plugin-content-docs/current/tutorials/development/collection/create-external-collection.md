...
- 您已将对象存储桶集成到 Zilliz Cloud。
    ...
    有关详细信息，请参见[阿里云对象存储](./integrate-with-alibaba-cloud-oss)和 [Amazon S3](./integrate-with-amazon-s3-cn)。
...
在执行 Search、Query、Get 和 Hybrid Search 等 DQL 操作之前，您需要先创建一个 Session，以指定 On-demand 集群的计算资源。有关详细信息，请参见[按需 DQL 操作](./dql-sessions-external-collection)。
