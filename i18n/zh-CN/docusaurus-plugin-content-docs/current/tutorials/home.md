...
# 自信构建，并为你的 AI 应用注入强劲动力\{#build-with-confidence-and-supercharge-your-ai-applications}

Zilliz Cloud 提供完全托管的 Milvus 服务，在兼顾安全性的同时，简化向量搜索应用的部署与扩展。

## 基本向量搜索\{#basic-vector-search}

执行近似最近邻（ANN）搜索，以查找与你的查询向量最相似的向量。[了解更多](./single-vector-search)。
...
在向量搜索之前应用元数据过滤器来缩小搜索范围，以提升结果相关性。[了解更多](./filtered-search)。
...
组合多个向量字段，以便跨文本、图像等进行多模态搜索。[了解更多](./hybrid-search)。
...
使用 BM25 相关性评分通过关键词匹配搜索文本，以实现精确的词项检索。[了解更多](./full-text-search)。
...
通过标量字段或主键进行过滤来检索实体，无需向量搜索。[了解更多](./get-and-scalar-query)。
...
1. 设置存储集成。
    ...
    将 AWS S3 bucket、Google Cloud Storage bucket 或 Microsoft Azure blob storage container 与 Zilliz Cloud 集成。

1. 创建一个[外部卷](./external-volumes)。

    使用某个路径或整个外部存储作为外部卷。外部卷是对集成存储中 bucket 或路径的只读引用，允许 Zilliz Cloud 就地访问你的数据，而无需复制或移动数据。 

1. [创建数据库](./create-database#create-database-in-on-demand-compute-or-public)。

    在按需计算中创建数据库。该数据库是项目级资源，由项目中的所有按需集群共享。 

1. 在数据库中创建外部 collection。

    将 collection 列映射到你的 Parquet 文件、Lance table、Iceberg table，或 0.56.0 版本起支持的 Vortex 文件。

1. 创建 index 并刷新 collection。

    为所有 vector 字段和可选 scalar 字段创建 index，然后刷新 collection，以便 Zilliz Cloud 为该 collection 创建元数据和 index 文件。刷新通常在亚秒级完成。

1. 开始探索你的数据。

    随后，你可以使用按需计算资源，对存储在外部存储中的数据进行 vector 搜索和 scalar 过滤。

## 自带 Vectors\{#bring-your-own-vectors}
...
1. 创建 collection。
    ...
    collection 是一个具有固定列和可变行的二维表。你可以创建外部 collection，将计算带到你的数据旁；也可以创建托管 collection，享受高性能 vector 搜索服务。

1. 导入数据。

    从本地文件或对象存储 bucket [导入数据](./data-import-export)。

1. 执行 vector 相似性搜索。

    [基础 vector 相似性搜索](./single-vector-search)可帮助你找到最相似的结果。

## 集成式 Embedding\{#integrated-embedding}
...
1. 设置模型提供商集成或部署托管模型。
    ...
    创建 AWS、GCP 或 Azure 存储集成，用于存储第三方模型提供商的凭证。或者，你也可以[部署](./hosted-models)托管模型。

1. 创建 collection 并配置 embedding 函数。

    创建一个至少包含一个 vector 字段和一个 VARCHAR 字段的托管 collection，并定义文本 embedding 函数。

1. 插入原始文本数据。

    [插入](./insert-entities)原始数据。Zilliz Cloud 会在摄取期间自动生成 vector embeddings。

1. 使用原始文本执行搜索。

    提供原始查询文本。Zilliz Cloud 会对查询进行 embedding，将其与存储的 vectors 进行比较，并[返回](./single-vector-search)最相关的结果。

## 从其他数据基础设施迁移\{#migrate-from-other-data-infra}

1. 连接到你的数据源。

    Zilliz Cloud 支持多种数据源，包括 Pinecone、MongoDB、Qdrant、PostgreSQL 等。请参阅[迁移](./migrations)[指](./migrations)[南](./migrations)。
...
1. 查看映射。
    ...
    设置并查看源数据和目标数据 schema 之间的映射关系。

## 备份与恢复\{#backup-and-restore}

1. 为你的集群或 collection 创建备份。

    备份是集群或 collection 在某一时间点的副本。你可以[手动](./create-backup)创建备份，或[设置备份策略](./schedule-automatic-backups)以进行计划备份。你还可以[将备份复制到其他区域](/docs/backup-to-other-regions)，以提升灾难恢复能力。
...
# 使用 Zilliz Cloud 走得更远\{#go-further-with-zilliz-cloud}
...
<Banner bannerText="找不到你要的内容？" bannerLinkText="试试 Ask AI" />
