...
# 自信构建，增强您的 AI 应用\{#build-with-confidence-and-supercharge-your-ai-applications}

Zilliz Cloud 提供完全托管的 Milvus 服务，在兼顾安全性的同时，简化向量搜索应用的部署和扩展。

## 基础向量搜索\{#basic-vector-search}

执行近似最近邻 (ANN) 搜索，以查找与查询向量最相似的向量。[了解更多](./single-vector-search)。
...
组合多个向量字段，在文本、图像等多种模态之间进行搜索。[了解更多](./hybrid-search)。
...
使用关键字匹配和 BM25 相关性评分来搜索文本，以实现精准的词项检索。[了解更多](./full-text-search)。
...
无需向量搜索，即可通过标量字段或主键过滤来检索实体。[了解更多](./get-and-scalar-query)。
...
选择一个项目[套餐](./select-zilliz-cloud-service-plans)，并在项目中创建不同部署选项的集群。
...
# 在 Zilliz Cloud 中使用您的数据\{#work-with-your-data-in-zilliz-cloud}

## 将计算资源带到您的数据旁\{#bring-compute-resources-to-your-data}
...
1. 创建一个[外部卷](./external-volumes)。
    ...
    使用一个路径或整个外部存储作为外部卷。外部卷是对已集成存储中存储桶或路径的只读引用，允许 Zilliz Cloud 在不复制或移动数据的情况下就地访问您的数据。 

1. [创建数据库](./create-database#create-database-in-on-demand-compute-or-public)。

    在按需计算中创建数据库。该数据库是一个项目级资源，由项目中的所有按需集群共享。 

1. 在数据库中创建外部集合。

    将集合列映射到您的 Parquet 文件、Lance 表、Iceberg 表，以及 0.56.0 起支持的 Vortex 文件。

1. 创建索引并刷新集合。

    为所有向量字段和可选标量字段创建索引，然后刷新集合，以便 Zilliz Cloud 为该集合创建元数据和索引文件。刷新通常在亚秒级完成。

1. 开始探索您的数据。

    然后，您可以使用按需计算资源，在存储于外部存储中的数据上开始向量搜索和标量过滤。
...
1. 创建集合。
    ...
    集合是一个具有固定列和可变行的二维表。您可以创建外部集合，将计算带到您的数据旁；也可以创建托管集合，以享受高性能向量搜索服务。
...
1. 执行向量相似性搜索。
    ...
    [基本向量相似性搜索](./single-vector-search)可帮助您找到最相似的结果。
...
1. 设置模型提供商集成或部署托管模型。
    ...
    创建 AWS、GCP 或 Azure 存储集成，以存储第三方模型提供商的凭据。或者，您可以[部署](./hosted-models)托管模型。

1. 创建集合并配置 embedding function。

    创建一个至少包含一个向量字段和一个 VARCHAR 字段的托管集合，并定义 text embedding function。

1. 插入原始文本数据。

    [插入](./insert-entities)原始数据。Zilliz Cloud 会在摄取期间自动生成向量嵌入。
...
1. 连接到您的数据源。
    ...
    Zilliz Cloud 支持各种数据源，包括 Pinecone、MongoDB、Qdrant、PostgreSQL 等。请参阅[迁移](./migrations)[指](./migrations)[南](./migrations)。
...
1. 查看映射。
    ...
    设置并查看源数据和目标数据 schema 之间的映射。

## 备份与恢复\{#backup-and-restore}

1. 为您的集群或集合创建备份。

    备份是集群或集合的时间点副本。您可以[手动](./create-backup)创建备份，或[设置备份策略](./schedule-automatic-backups)以进行计划备份。您还可以[将备份复制到其他区域](/docs/backup-to-other-regions)，以提升灾难恢复能力。

1. （可选）将备份导出到对象存储服务。

    您可以将已创建的[备份文件导出](./export-backup-files)到 AWS S3 或 Azure Blob Storage。
...
# 使用 Zilliz Cloud 更进一步\{#go-further-with-zilliz-cloud}
...
<Banner bannerText="找不到您要查找的内容？" bannerLinkText="试试 Ask AI" />
