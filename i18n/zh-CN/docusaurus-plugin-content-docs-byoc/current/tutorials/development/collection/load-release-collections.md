...
notebook: FALSE
description: "对 Collection 执行 Load 操作是在 Collection 中进行 Search 和 Query 的前提条件。本节主要介绍如何 Load 和 Release Collection。 | BYOC"
type: origin
...
需要注意的是，当您选择只加载部分字段时，只有在 `load_fields` 中的字段可以作为过滤条件（filter）和输出字段（Output Fields）使用。请务必在 `load_fields` 中包含主键和至少一个已创建索引的向量字段。
