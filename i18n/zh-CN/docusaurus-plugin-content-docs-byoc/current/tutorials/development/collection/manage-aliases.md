...
notebook: FALSE
description: "Zilliz Cloud 为 Collection 提供了 Alias 管理能力。本节介绍如何创建、查看和删除 Alias。 | BYOC"
type: origin
...
Zilliz Cloud 为 Collection 提供了 Alias 管理能力。本节介绍如何创建、查看和删除 Alias。

## 为何使用 Alias\{#why-use-an-alias}

通过为指定 Collection 绑定不同的别名，您可以在不修改应用代码的情况下，便捷地切换到不同的 Collection。

假设您的应用需要查询的 Collection 绑定了一个名为 `prod_data`的 Alias。当您需要使用新数据时，您可以按照如下步骤以不中断业务的方式完成该任务：
...
此时，您的应用仍然可以将业务请求发送到 `prod_data`，业务运行完全不受影响。该机制让数据无缝更新成为可能，简化了向量检索服务的蓝绿部署流程。
...
您可以参考如下代码查看分配给指定 Collection 的所有 Alias。
...
您可以参考如下代码查看指定 Alias 绑定的 Collection。
...
您可以参考如下代码将已经分配给某个 Collection 的 Alias 重新分配给另一个 Collection。
