...
notebook: FALSE
description: "截断 Collection 会移除 Collection 中的所有 Entity，同时保留 Collection 的 Schema、约束和 Index。相比删除 Entity，截断 Collection 更高效，因为它会从 Search 和 Query 中隐藏当前时间戳之前已 flush 的所有 Entity，并在后台删除这些 Entity。 | BYOC"
type: origin
...
截断 Collection 会移除 Collection 中的所有 Entity，同时保留 Collection 的 Schema、约束和 Index。相比删除 Entity，截断 Collection 更高效，因为它会从 Search 和 Query 中隐藏当前时间戳之前已 flush 的所有 Entity，并在后台删除这些 Entity。
