...
notebook: FALSE
description: "本文展示了如何统计 Collection 中的 Entity 数量，并解释了 Entity 计数可能与实际数字不同的原因。 | BYOC"
type: origin
...
本文展示了如何统计 Collection 中的 Entity 数量，并解释了 Entity 计数可能与实际数字不同的原因。
...
- **查询以 count(&ast;) 作为输出字段**
    ...
    在接收到此类查询时，Zilliz Cloud 会向查询节点发送请求，并统计已加载到内存中的 Entity 数量。
...
- **使用 get_collection_stats()**
    ...
    虽然您可以使用上述方法获取 Collection 的确切计数，但不建议在所有场景中都使用它。此过程本质上是一次查询，频繁调用可能会导致网络抖动，或者影响与您的业务相关的搜索和查询。

    如果统计结果的准确性不是主要考虑因素，您应该使用 `get_collection_stats()` 和 `get_partition_stats()`。虽然此调用提供的是估计的 Entity 计数，但您无需加载目标 Collection 即可运行它，而且成本小到可以忽略不计，因为它只是报告内部跟踪器记录的内容。
...
如上文所述，`get_collection_stats()` 返回 Collection 中 Entity 的估计数量，该数量可能与实际 Entity 计数不同。您可以将此作为参考，而无需加载 Collection。
...
您可以在集群的**实体计数**和**已加载实体（近似值）**的**指标**选项卡中找到它们。这两个值都是估算值。曲线中的值是通过[使用](./count-entities#use-get_collection_stats)[`get_collection_stats()`](./count-entities#use-get_collection_stats)获取的。如果没有进一步的数据插入和删除，**实体计数**曲线最终将反映当前 Collection 中 Entity 的实际数量。
...
您可以在 Collection 的详情选项卡中找到其实际 Entity 计数。此值是通过使用[查询并将](./count-entities#query-with-count-as-the-output-field)[`count(*)`](./count-entities#query-with-count-as-the-output-field)[作为输出字段](./count-entities#query-with-count-as-the-output-field)获得的。
...
您还可以使用 Collection 的 **Partitions** 选项卡来查找其子 Partition 中已加载 Entity 的估计数量。此值通过使用 `get_partition_stats()` 获得。
...
- **在我插入或删除一些 Entity 后，Collection 的指标选项卡上的实体计数曲线为何没有变化？**
    ...
    **实体计数**曲线中的值是在特定时间点估计的，由于所有数据操作都是异步的，所以在它们反映到曲线中之前可能会有延迟。

- **为什么在 Collection 的“Partitions”选项卡的“实体计数（近似）”列中显示的值在我插入或删除一些 Entity 后没有改变？**

    列出的 Partition 所显示的值均为估算值。由于所有数据操作都是异步的，因此在它们反映到曲线中之前可能会有延迟。

- **为什么 Collection 概述选项卡中“已加载实体”显示的值不能反映 Collection 中 Entity 的实际数量？**
