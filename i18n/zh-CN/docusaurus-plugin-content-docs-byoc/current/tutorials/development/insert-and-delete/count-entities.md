...
notebook: FALSE
description: "本文展示了如何统计 Collection 中的 Entity 数量，并解释了 Entity 计数可能与实际数量不同的原因。 | BYOC"
type: origin
...
- **使用 get_collection_stats()**
    ...
    虽然您可以使用上述方法获取 Collection 的确切计数，但不建议在所有场合都使用它。这个过程本质上是一个查询，频繁调用可能会导致网络抖动，或者影响与您的业务相关的搜索和查询。
...
- **为什么 Collection 概述选项卡中 “已加载实体” 显示的值不能反映 Collection 中 Entity 的实际数量？**
    ...
    在**已加载实体**中显示的值是准确的。如果此值与从典型查询获得的 Entity 数量之间存在差距，则 Collection 中的某些 Entity 可能具有相同的主键。
