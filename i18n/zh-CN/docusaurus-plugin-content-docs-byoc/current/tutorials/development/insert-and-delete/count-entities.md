...
- **查询以 count(&ast;) 作为输出字段**
    ...
    要获取 Collection 中 Entity 的确切数量，您应使用此方法，并确保：
...
- **使用 get_collection_stats()**
    ...
    虽然您可以使用上述方法获取 Collection 的确切计数，但不建议在所有场景中都使用它。这个过程本质上是一次查询，频繁调用可能会导致网络抖动，或影响与业务相关的搜索和查询。

    如果统计结果的准确性不是主要考虑因素，您应使用 `get_collection_stats()` 和 `get_partition_stats()`。虽然此调用提供的是估计的 Entity 计数，但您无需加载目标 Collection 即可运行它，而且成本小到可以忽略不计，因为它只是报告内部跟踪器记录的内容。
...
您还可以使用 Collection 的**分区**选项卡来查找其子分区中已加载 Entity 的估计数量。此值通过使用 `get_partition_stats()` 获得。
...
- **在我插入或删除一些 Entity 后，Collection 的指标标签页上的实体计数曲线为何没有变化？**

    **实体计数**曲线中的值是在特定时间点估计的。由于所有数据操作都是异步的，因此在它们反映到曲线中之前可能会有延迟。
