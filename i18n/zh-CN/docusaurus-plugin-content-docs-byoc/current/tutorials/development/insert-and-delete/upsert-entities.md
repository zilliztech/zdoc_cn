...
您可以使用 `upsert` 操作来插入新 Entity 或更新现有 Entity，具体取决于请求中提供的主键是否已存在于 Collection 中。如果未找到主键，则执行插入操作。否则，将执行更新操作。

Milvus 中的 upsert 操作可以在**覆盖**或**合并**模式下工作。

### 在覆盖模式下执行 Upsert\{#update-in-override-mode}

在覆盖模式下工作的 upsert 请求。当收到针对现有 Entity 的 `upsert` 请求时，Zilliz Cloud 会插入请求载荷中携带的数据，同时删除数据中指定的具有原始主键的现有 Entity。

![Af76wmsgbh7C0cbfQ36cTbUonpe](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/Af76wmsgbh7C0cbfQ36cTbUonpe.png)

如果目标 Collection 的主键字段启用了 `autoID`，‎`upsert` 请求仍然必须包含目标 Entity 的主键。Zilliz Cloud 会使用请求中提供的主键定位要被替换的 Entity，并在插入前为请求载荷中的新数据生成一个新的主键。

对于启用了可空的字段，如果不需要更新，则可以在 `upsert` 请求中省略它们。

### 在合并模式下执行 Upsert\{#update-in-merge-mode}

您还可以使用 `partial_update` 标志，使 `upsert` 请求以合并模式工作。这允许您仅在请求载荷中包含需要更新的字段。
...
- **在 Dynamic Field 中 Upsert 键**。
    ...
    假设您已在示例 Collection 中启用 Dynamic Field，并且 Entity Dynamic Field 中的键值对类似于 `{"author": "John", "year": 2020, "tags": ["fiction"]}`。

    当您使用键（如 `author`、`year` 或 `tags`）upsert Entity，或添加其他键时，请注意：

    - 如果在禁用 `partial_update` 的情况下进行 `upsert` 操作，默认行为是**覆盖**。这意味着 Dynamic Field 的值将被请求中包含的所有非 schema 定义字段及其值覆盖。

        例如，如果请求中包含的数据是 `{"author": "Jane", "genre": "fantasy"}`，则目标 Entity Dynamic Field 中的键值对将更新为该数据。

    - 如果在启用 `partial_update` 的情况下进行 upsert 操作，默认行为是**合并**。这意味着 Dynamic Field 的值将与请求中包含的所有非 schema 定义字段及其值进行合并。

        例如，如果请求中包含的数据是 `{"author": "John", "year": 2020, "tags": ["fiction"]}`，则目标 Entity Dynamic Field 中的键值对在 upsert 后将变为 `{"author": "Jane", "year": 2020, "tags": ["fiction"], "genre": "fantasy"}`。
...
- **Upsert** `ARRAY` **字段。**
    ...
    - `ARRAY_APPEND` 将请求载荷中的元素追加到现有数组中。

    - `ARRAY_REMOVE` 从现有数组中移除与请求载荷中的值匹配的所有元素。
...
如果 Collection 中存在这三个 Entity，它们将被 upsert 请求中包含的 Entity 覆盖。
