...
对于启用了 nullable 的字段，如果不需要更新，则可以在 `upsert` 请求中省略它们。
...
在本节中，我们将向名为 `my_collection` 的 Collection 中 Upsert Entity。这个 Collection 只有四个字段，分别名为 `id`、`vector`、`title` 和 `issue`。`id` 字段是主键字段，而 `title` 和 `issue` 字段是标量字段。
