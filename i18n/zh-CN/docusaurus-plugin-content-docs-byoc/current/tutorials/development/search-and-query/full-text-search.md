...
向 Collection 插入数据后，可以使用原始查询文本执行 Full Text Search。Milvus 会自动将查询文本转换为稀疏向量，并使用 BM25 算法按相关性对匹配的搜索结果进行排序。
...
| `params.level` | 使用简化的调优方式控制搜索精度。更多详情，请参考[召回率调优](./tune-recall-rate) |
| `data` | 原始查询文本。Zilliz Cloud 使用 BM25 Function 自动将查询请求中的文本转换为对应的稀疏向量。因此，请勿提供预生成的向量。 |
| `anns_field` | 用于存储 Milvus 内部自动生成的稀疏向量的向量字段名称。 |
| `output_fields` | 要在搜索结果中返回的字段名列表。支持**除包含 BM25 生成嵌入的稀疏向量字段之外**的所有字段。常见的输出字段包括主键字段（例如 id）和原始文本字段（例如 text）。更多信息请参考 [FAQ](./full-text-search#faqs)。 |
| `limit` | 返回的最大匹配结果数量。 |

## 常见问题\{#faqs}

### 在 Full Text Search 中，我能否输出或访问由 BM25 Function 生成的稀疏向量？\{#bm25}

不能。BM25 Function 生成的稀疏向量在 Full Text Search 中无法直接访问或输出。详情如下：

- BM25 Function 会在内部生成稀疏向量，用于排序和检索
