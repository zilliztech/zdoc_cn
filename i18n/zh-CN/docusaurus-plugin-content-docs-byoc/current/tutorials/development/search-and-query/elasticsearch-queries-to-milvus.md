...
notebook: FALSE
description: "基于 Apache Lucene 构建的 Elasticsearch 是领先的开源搜索引擎。然而，它在现代 AI 应用程序中面临挑战，包括高更新成本、较差的实时性能、低效的分片管理、非云原生设计以及过高的资源需求。作为云原生向量数据库，Zilliz Cloud 通过解耦的存储和计算、高维数据的高效索引以及与现代基础设施的无缝集成克服了这些问题，并为 AI 工作负载提供了卓越的性能和可扩展性。 | BYOC"
type: origin
...
基于 Apache Lucene 构建的 Elasticsearch 是领先的开源搜索引擎。然而，它在现代 AI 应用程序中面临挑战，包括高更新成本、较差的实时性能、低效的分片管理、非云原生设计以及过高的资源需求。作为云原生向量数据库，Zilliz Cloud 通过解耦的存储和计算、高维数据的高效索引以及与现代基础设施的无缝集成克服了这些问题，并为 AI 工作负载提供了卓越的性能和可扩展性。

本文将通过各种查询转换示例，帮助您将代码库从 Elasticsearch 迁移到 Zilliz Cloud。
...
<table>
   ...
   <tr>
     ...
     <td><p>文本匹配</p></td>
     <td><p>Zilliz Cloud 提供一种不参与相似性评分的文本匹配能力。该能力与 Elasticsearch 的 match 子句能力相当。</p><p>请注意，Elasticsearch 中所有的 match 子句都会产生相似性分数。因此，Elasticsearch 中不存在与 Zilliz Cloud 中仅进行文本匹配且不输出相似性分数的能力完全对应的功能。</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td rowspan="6"><p>当左侧相关子句在 Elasticsearch 的过滤器上下文中使用时，与 Zilliz Cloud 提供的能力相同或相似。</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>当左侧相关子句在 Elasticsearch 的过滤器上下文中使用时，与 Zilliz Cloud 提供的能力相同或相似。</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>Zilliz Cloud 提供更多重排策略。</p></td>
   </tr>
</table>

## 全文查询（Full-text queries）\{#full-text-queries}

在 Elasticsearch 中，全文检索允许您检索经过分析的文本字段，例如电子邮件正文。这类检索会使用与字段索引相同的分析器，对检索字符串进行处理。
...
在上面的示例中，`message_sparse` 是一个从名为 `message` 的 VarChar 字段派生出的稀疏向量字段。Zilliz Cloud 使用 BM25 embedding 模型将 `message` 字段中的值转换为稀疏向量 embedding，并将其存储在 `message_sparse` 字段中。当接收到检索请求时，Zilliz Cloud 使用相同的 BM25 模型对纯文本查询进行 embedding，并执行稀疏向量检索，同时返回 `output_fields` 参数中指定的 `id` 和 `message` 字段，以及相应的相似度分数。  

要使用此功能，必须在 `message` 字段上启用分析器，并定义一个函数，从中派生出 `message_sparse` 字段。相关操作可以参考[全文搜索](./full-text-search)。
...
与在 Match 查询中使用 Zilliz Cloud 生成稀疏向量 embedding 到派生字段不同，上述示例将 `message` 字段中的值转换为稠密向量，并使用 `TEXT_MATCH` 过滤器来确保检索结果与“Jamaica”密切相关。具体来说，Zilliz Cloud 首先执行文本匹配，分析并过滤 `message` 字段中包含“Jamaica”一词的 Entity，然后在过滤后的子集中执行向量检索，以提高检索召回率。
...
在 Zilliz Cloud 中，您可以查找指定字段值以特定前缀开头的 Entity，方法如下：
...
在 Zilliz Cloud 中，您可以查找特定字段值位于指定范围内的 Entity，方法如下：
...
Zilliz Cloud 的过滤条件不支持通配符。不过，您可以使用 `LIKE` 操作符来实现类似效果，方法如下：
...
在 Elasticsearch 中，布尔查询是一种组合查询，可包含多个布尔查询和其他查询。
...
Zilliz Cloud 作为一款专门的向量数据库，使用索引类型来优化向量检索。通常，它优先采用近似最近邻（ANN）检索来处理高维向量数据。虽然使用 `FLAT` 索引类型的暴力 kNN 检索能够提供精确结果，但它既耗时又耗资源。相比之下，使用 `AUTOINDEX` 或其他索引类型的 ANN 检索，能够更好地平衡速度和准确性，提供比 KNN 更快的性能，并且更节省资源。关于索引类型和 AUTOINDEX 的相关内容，可以参考[管理 Index](./undefined) 和 [AUTOINDEX](./autoindex-explained)。
...
- 在 `vector` 字段上使用提供的查询向量进行 kNN 检索。
...
这些检索的结果分别执行后，通过倒数秩融合（RRF）排序器进行组合和重排。混合检索返回重排列表中的前 10 个 Entity。  

与 Elasticsearch 的 RRF 排序（基于标准文本查询和 kNN 检索的结合）不同，Zilliz Cloud 结合了稀疏向量和稠密向量检索，提供了一种专门优化多模态数据的独特混合检索方法。

## 总结\{#recap}

在本文中，我们介绍了如何将经典 Elasticsearch 查询等效地转换到 Zilliz Cloud 中，包括词项级查询、布尔查询、全文查询和向量查询。如果您对转换其他 Elasticsearch 查询有任何问题，欢迎随时联系我们。
