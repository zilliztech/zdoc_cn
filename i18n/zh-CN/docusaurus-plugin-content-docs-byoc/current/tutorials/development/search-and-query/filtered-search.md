...
这种方式显著减少了标量过滤需要处理的 Entity 数量，使得其尤其适用于处理过滤条件较为复杂的场景。

需要注意的是，迭代器每次只处理一个 Entity，并以串行的方式进行迭代。使用迭代 Filtered Search 可能会拉长请求处理时间，或在需要处理的 Entity 较大时出现潜在的性能问题。

## 操作示例\{#examples}

本节将结合具体的示例介绍如何进行 Filtered Search。在如下示例中，假设 Collection 已经存放了如下 10 条 Entity。每个 Entity 都有 **id**、**vector**、**color** 和 **likes** 这几个字段。
