...
| --- | --- | --- |
| `L2` | 较小的 L2 距离表示更高的相似度。 | 要排除结果中最近的向量，请确保：<br/>`0` &lt;= `range_filter` &lt;= distance < `radius` < `∞` |
| `IP` | 较大的 IP 距离表示更高的相似度。 | 要排除结果中最近的向量，请确保：<br/>`-1` &lt;= `radius` < distance &lt;= `range_filter` &lt;= `1` |
| `COSINE` | 较大的 cosine 值表示更高的相似度。 | 要排除结果中最近的向量，请确保：<br/>`-1` &lt;= `radius` < distance &lt;= `range_filter` &lt;= `1` |
| `JACCARD` | 较小的 Jaccard 距离表示更高的相似度。 | 要排除结果中最近的向量，请确保：<br/>`0` &lt;= `range_filter` &lt;= distance < `radius` &lt;= `1` |
| `HAMMING` | 较小的 Hamming 距离表示更高的相似度。 | 要排除结果中最近的向量，请确保：<br/>`0` &lt;= `range_filter` &lt;= distance < `radius` &lt;= `dim(vector)` |
...
如果查询向量已经在目标 Collection 中存在，可以考虑使用 `ids` 参数，从而让 Milvus 在搜索前从 Collection 中自动获取查询向量。更多内容，请阅读 [Primary Key Search](./primary-key-search)。
