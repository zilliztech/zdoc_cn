...
- `slop`（可选）：一个整数，用于指定匹配词元中允许的最大位置数。
    ...
    - `2`：允许更大的灵活性，包括词序颠倒或中间最多两个词。示例：匹配目标为 **"machine learning"** 的过滤表达式将匹配 **"learning machine"**（词序颠倒）或 **"machine quickly boosts learning"**（**"machine"** 和 **"learning"** 之间有两个词）。
...
在这里，我们允许容差为 1。该过滤器应用于包含短语 **"learning machine"** 的文档，并具有一定灵活性。
...
此示例允许容差为 2，这意味着在 **"machine"** 和 **"learning"** 这两个词之间最多允许有两个额外内容（或词的位置发生变换）。
