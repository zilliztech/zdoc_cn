...
然而，在实际场景中，严格的短语匹配可能过于死板。您可能希望匹配类似 *"…machine learning models widely adopted in robotics…"* 这样的文本。在这里，相同的关键词存在，但并非相邻或按原始顺序排列。为处理这种情况，短语匹配支持 `slop` 参数，该参数可引入灵活性。`slop` 值定义了短语中各词项之间允许的位置偏移数量。例如，当 `slop` 值为 1 时，对 **"machine learning"** 的查询可以匹配类似 *"...machine deep learning..."* 这样的文本，其中一个词（**"deep"**）分隔了原始词项。
...
如果您的文本数据使用特定语言或格式，可以通过 `analyzer_params` 参数配置自定义分析器（例如 `{ "type": "english" }` 或 `{ "type": "jieba" }`）。
...
创建 Collection 后，在使用短语匹配之前，请确保完成以下必要步骤：
...
在这里，我们允许容差为 1。该过滤器应用于包含短语 **"learning machine"** 的文档，并具有一定的灵活性。
...
此示例允许容差为 2，这意味着在 **"machine"** 和 **"learning"** 这两个词之间最多允许有两个额外内容（或位置变换的词）。
...
- `filter` 中的转义规则：
    ...
    - 表达式中用双引号或单引号括起来的字符会被解释为字符串常量。如果字符串常量包含转义字符，则必须使用转义序列表示这些转义字符。例如，使用 `\\` 表示 `\`，使用 `\\t` 表示制表符 `\t`，以及使用 `\\n` 表示换行符。
