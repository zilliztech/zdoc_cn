...
然而，在实际场景中，严格的短语匹配可能过于死板。您可能希望匹配类似 *"…machine learning models widely adopted in robotics…"* 这样的文本。在这里，相同的关键词存在，但并非相邻，也不是按原始顺序排列。为处理这种情况，短语匹配支持 `slop` 参数，该参数引入了灵活性。`slop` 值定义了短语中各词项之间允许的位置偏移数量。例如，当 `slop` 值为 1 时，对 **"machine learning"** 的查询可以匹配类似 *"...machine deep learning..."* 这样的文本，其中一个词（**"deep"**）分隔了原始词项。
...
默认情况下，Zilliz Cloud 使用 [standard analyzer](./standard-analyzer)，该 Analyzer 会根据空格和标点对文本进行分词，并将文本转换为小写。

如果您的文本数据使用特定语言或格式，可以通过 `analyzer_params` 参数配置自定义 Analyzer（例如 `{ "type": "english" }` 或 `{ "type": "jieba" }`）。
...
- `field_name`：您执行短语匹配的 `VARCHAR` 字段的名称。

- `phrase`：要搜索的确切短语。

- `slop`（可选）：一个整数，指定匹配词元中允许的最大位置数。
...
使用 `query()` 方法时，**PHRASE_MATCH** 用作标量过滤条件。此时，查询仅返回包含指定短语（允许有一定间隔）的文档。
...
在这里，我们允许 1 的容差。该过滤器应用于包含短语 **"learning machine"** 的文档，并具有一定的灵活性。
...
此示例允许 2 的容差，这意味着在 **"machine"** 和 **"learning"** 这两个词之间最多允许有两个额外内容（或发生位置变换的词）。
...
在此示例中，slop 为 3 提供了更大的灵活性。过滤器搜索 **"machine learning"**，允许单词之间最多相隔三个词元位置。
...
- 为字段启用短语匹配会触发倒排索引的创建，这会消耗存储资源。在决定启用此功能时，请考虑对存储的影响，因为存储空间占用会根据文本大小、唯一词元以及所使用的 Analyzer 而有所不同。

- 在 Schema 中定义 Analyzer 后，其设置将对该 Collection 永久生效。如果您认为其他 Analyzer 更适合您的需求，可以考虑删除现有 Collection，并使用所需的 Analyzer 配置创建一个新 Collection。

- 短语匹配性能取决于文本的分词方式。在将 Analyzer 应用于整个 Collection 之前，请使用 `run_analyzer` 方法查看分词输出。有关更多信息，请参阅 [Analyzer 概述](./analyzer-overview)。

- `filter` 中的转义规则：

    - 表达式中用双引号或单引号括起来的字符会被解释为字符串常量。如果字符串常量包含转义字符，则必须使用转义序列表示这些转义字符。例如，使用 `\\` 表示 `\`，使用 `\\t` 表示制表符 `\t`，使用 `\\n` 表示换行符。

    - 如果字符串常量用单引号括起来，常量内的单引号应表示为 `\\'`，而双引号可以表示为 `"` 或 `\\"`。示例：`'It\\'s milvus'`。

    - 如果字符串常量用双引号括起来，常量内的双引号应表示为 `\\"`，而单引号可以表示为 `'` 或 `\\'`。示例：`"He said \\"Hi\\""`。
