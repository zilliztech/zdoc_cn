...
Text Match 还可以用于 Query 操作中的标量过滤。通过在 `query()` 方法的 `expr` 参数中指定 `TEXT_MATCH` 表达式，可以检索到匹配给定关键词的文档。
...
- 为字段启用 Text Match 时，会创建倒排索引，这会占用存储资源。启用该功能时，请考虑存储影响，因为这取决于文本大小、唯一分词数量以及使用的分词器。
