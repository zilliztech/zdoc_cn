...
Text Match 还可以用于查询操作中的标量过滤。通过在 `query()` 方法的 `expr` 参数中指定 `TEXT_MATCH` 表达式，可以检索匹配给定关键词的文档。

以下示例查询 `text` 字段中包含关键词 `"keyword1"` 和 `"keyword2"` 的文档：
...
- 为字段启用 Text Match 时，会创建倒排索引，这会占用存储资源。启用该功能时，请考虑存储方面的影响，因为这取决于文本大小、唯一分词数量以及所使用的分词器。

- 一旦在 Schema 中定义了分词器配置，该配置就会固定在 Collection 上。如果您认为其他分词器更适合您的需求，可以考虑删除现有 Collection，并创建一个包含所需分词器配置的新 Collection。
