...
notebook: FALSE
description: "Zilliz Cloud 提供了一套丰富的基本操作符，可帮助您高效地过滤和查询数据。通过这些操作符，您可以根据标量字段、数字计算、逻辑条件等细化搜索条件。了解如何使用这些操作符对于构建精确查询和最大限度提高搜索效率至关重要。 | BYOC"
type: origin
...
Zilliz Cloud 提供了一套丰富的基本操作符，可帮助您高效地过滤和查询数据。通过这些操作符，您可以根据标量字段、数字计算、逻辑条件等细化搜索条件。了解如何使用这些操作符对于构建精确查询和最大限度提高搜索效率至关重要。
...
这对于希望检查某个参数值是否位于某个指定集合中非常有用。

### 示例 2：使用 `LIKE` 操作符进行模式匹配\{#example-2-using-like-for-pattern-matching}

对于文本字段，可以使用 `LIKE` 操作符进行模式匹配。具体来说，您可以检查某个文本字段的值是否在前缀、后缀或中缀等指定位置上包含某个子字符串。在 `LIKE` 操作符中，您可以使用 `%` 作为通配符，用于匹配零个或多个字符。

### 前缀匹配\{#prefix-match-starts-with}

如果需要进行前缀匹配，可以将 `%` 通配符放在需要匹配的子字符串后面。如下面的例子所示，如果需要找出 `name` 字段值以 `Prod` 开头的所有 Entity，可以使用如下过滤表达式。
...
对于后缀匹配，可以将 `%` 通配符放在需要匹配的子字符串前面。例如，需要找出 `name` 字段以 `XYZ` 结尾的所有 Entity，可以使用如下过滤表达式。
...
### 示例 1：使用取余（`%`）操作符\{#example-5-using-modulus-percent}
...
可以使用如下过滤表达式找出所有 `color` 字段值不为 `green` 的 Entity。
...
如果 JSON 字段值中某些元素（例如，某些键值）为 `null`，该字段的值仍为非空。例如，某个 Entity 的 JSON 字段取值为 `\{"metadata": \{"category": None, "price": 99.99}}`。虽然 `category` 的值为 `None`，但该 JSON 字段的取值不会被当作 `null`。
...
Zilliz Cloud 集群中的基本操作符用途广泛，可用于标量字段，也可有效地用于 JSON 和 ARRAY 字段中的键和索引。
