...
notebook: FALSE
description: "Zilliz Cloud 提供了一套丰富的基本操作符，可帮助您高效地过滤和查询数据。通过这些操作符，您可以根据标量字段、数值计算、逻辑条件等细化搜索条件。了解如何使用这些操作符对于构建精确查询和最大限度提高搜索效率至关重要。 | BYOC"
type: origin
...
Zilliz Cloud 提供了一套丰富的基本操作符，可帮助您高效地过滤和查询数据。通过这些操作符，您可以根据标量字段、数值计算、逻辑条件等细化搜索条件。了解如何使用这些操作符对于构建精确查询和最大限度提高搜索效率至关重要。
...
这在需要检查某个参数值是否属于某个指定集合时非常有用。

### 示例2：使用 `LIKE` 操作符进行模式匹配\{#example-2-using-like-for-pattern-matching}

对于文本字段，可以使用 `LIKE` 操作符进行模式匹配。具体来说，您可以检查某个文本字段的值是否在前缀、后缀或中缀等指定位置包含某个子字符串。在 `LIKE` 操作符中，可以使用 `%` 作为通配符，用于匹配零个或多个字符。

### 前缀匹配\{#prefix-match-starts-with}

如果需要进行前缀匹配，可以将 `%` 通配符放在需要匹配的子字符串后面。如下例所示，如果需要找出 `name` 字段值以 `Prod` 开头的所有 Entity，可以使用如下过滤表达式。

```python
filter = 'name LIKE "Prod%"'
```

这将匹配任何名称以 "Prod" 开头的产品，例如 "Product A"、"Product B" 等。

该过滤表达式将匹配 `name` 字段值为 `ProductA`、`ProductB` 等以 `Prod` 开头的所有 Entity。

### 后缀匹配\{#suffix-match-ends-with}

对于后缀匹配，可以将 `%` 通配符放在需要匹配的子字符串前面。例如，如果需要找出 `name` 字段以 `XYZ` 结尾的所有 Entity，可以使用如下过滤表达式。
...
进行中缀匹配，即匹配包含指定模式文本的字段值时，可以在模式文本前后各添加一个 `%` 通配符。例如，如果需要找出 `name` 字段包含 `Pro` 字样的所有 Entity，可以使用如下过滤表达式：
...
您可以使用算术操作符，根据涉及数值字段的算术计算式创建过滤条件。
...
- `IS NOT NULL` 判断 Entity 中指定字段的取值是否不为空，即 Entity 在该字段上的值为一个合法值。
...
在 `VARCHAR` 类型字段中，Zilliz Cloud 不会将 `""` 识别为 `null` 值。
...
如果 JSON 字段值中的某些元素（例如某些键值）为 `null`，该字段的值仍为非空。例如，某个 Entity 的 JSON 字段取值为 `\{"metadata": \{"category": None, "price": 99.99}}`。虽然 `category` 的值为 `None`，但该 JSON 字段的取值不会被当作 `null`。
...
Zilliz Cloud 对过滤条件中的匹配值列表没有长度限制。但是过长的列表会显著影响查询性能。因此，当您的过滤条件中匹配值列表较长，或过滤表达式较为复杂且包含大量元素时，推荐使用[过滤表达式模板](./filtering-templating)以提升查询性能。
