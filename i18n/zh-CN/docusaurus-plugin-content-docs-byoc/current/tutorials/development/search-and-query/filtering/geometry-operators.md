...
notebook: FALSE
description: "支持一组用于在 `GEOMETRY` 字段上进行过滤的操作符，这些操作符对于管理和分析几何数据至关重要。这些操作符能够实现有效的空间查询，使您能够根据对象之间的几何关系检索实体。 | BYOC"
type: origin
...
支持一组用于在 `GEOMETRY` 字段上进行过滤的操作符，这些操作符对于管理和分析几何数据至关重要。这些操作符能够实现有效的空间查询，使您能够根据对象之间的几何关系检索实体。

所有 Geometry 操作符都通过接受两个几何参数来运行：collection 中定义的 `GEOMETRY` 字段名称，以及以[众所周知的文本](https://en.wikipedia.org/wiki/Well-known_text_representation_of_geometry)（WKT）格式表示的目标几何对象。
...
您有一组预定义的兴趣点（例如地标），并且想通过其精确坐标快速找到特定的地标。
