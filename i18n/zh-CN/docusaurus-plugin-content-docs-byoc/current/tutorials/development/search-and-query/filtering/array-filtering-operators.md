...
ARRAY 操作符允许 Zilliz Cloud 集群中的 ARRAY 类型字段进行精细查询。这些操作符包括：
...
假设有名为 `history_temperatures` 的字段包含了各地气象站上报的历年最低气温。您可以使用如下过滤表达式找出最低气温列表中包含 23 度的所有 Entity。
...
该操作符可确保指定列表中的所有元素都出现在 ARRAY 字段中。当需要匹配数组中包含多个值的 Entity 时，该操作符非常有用。

**示例**

您可以使用如下过滤表达式找出最低气温列表中同时包含 23 度和 24 度的所有 Entity。
...
您可以使用如下过滤表达式找出最低气温列表中包含 23 度或 24 度的所有 Entity。
...
该操作符允许您根据 ARRAY 字段中的元素个数进行过滤。这对于找出包含特定数量元素的 Entity 十分有用。
