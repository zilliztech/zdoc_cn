...
Zilliz Cloud 并不要求用户将原子结构化事件转换为扁平数据模型，而是引入了 Struct Array，其中 Array 中的每个 Struct 都可以包含标量和向量，从而保留语义完整性。
...
Struct Array 字段允许您将一组有序的 Structs 存入其中，每个 Struct 都带有自己的标量和向量数据。这使其尤其适用于存储如下类型的数据：

- **多层嵌套数据**：具有多个子记录的父实体，例如包含许多文本块的书籍，或包含许多带注释帧的视频。

- **多模态向量嵌入**：每个 Struct 可以容纳多个向量，例如文本嵌入加上图像嵌入，以及元数据。

- **时序或顺序数据**：Array 字段中的 Struct 天然适合表示时间序列或逐步事件。

与存储 JSON 数据块或跨多个 Collection 拆分数据的传统解决方法不同，Struct Array 在 Zilliz Cloud 中提供原生模式约束、向量索引和高效存储。

## Schema 设计指南\{#schema-design-guidelines}

除了 [Schema 设计指南](./schema-design-hands-on)中提及的设计原则之外，在数据模型设计中引入 Struct Array 之前，您还需要考虑如下因素：
...
建议您仅包含用于检索或显示的字段，以保持 Struct Schema 简洁。避免因包含未使用的元数据而导致 Schema 臃肿。

### 谨慎设置 Max Capacity\{#set-the-max-capacity-thoughtfully}

每个 Array 字段都有一个属性，用于指定每个 Struct Array 字段所能容纳的最大元素数量。请根据实际情况的上限来设置此属性。例如，每个文档有 1000 个文本块，或每个驾驶场景有 100 个操作。

过高的值会浪费内存，您需要进行一些计算来确定 Array 字段中 Struct 的最大数量。

### 为 Struct 中的向量字段创建索引\{#index-vector-fields-in-structs}

索引对于向量字段是必需的，包括集合中的向量字段和在 Struct 中定义的向量字段。对于 Struct 中的向量字段，您应该使用 `AUTOINDEX` 作为索引类型，使用 `MAX_SIM` 系列相似度类型作为度量类型。
...
由 [Turing Motors](https://tur.ing/posts/s1QUA1uh) 推出并被 2025 年冬季计算机视觉应用会议（WACV）接受的综合视觉-语言-行动（CoVLA）数据集，为在自动驾驶中训练和评估视觉-语言-行动（VLA）模型提供了丰富基础。每个数据点（通常是一个视频片段）不仅包含原始视觉输入，还包含如下结构化说明信息：
...
该数据集多层级、多模态的自然属性，使其非常适合 Struct Array 的设计初衷。关于 CoVLA 数据集的更多信息，可以参考 [CoVLA 数据集官网](https://turingmotors.github.io/covla-ad/)。

### 步骤 1：将数据集映射到 Collection Schema\{#step-1-map-the-dataset-into-a-collection-schema}

CoVLA 数据集是一个大规模、多模态的驾驶数据集，包含 10000 个视频片段，总时长超过 80 小时。它以 20Hz 的帧率采样，并为每一帧标注详细的自然语言描述，以及车辆状态信息和检测到的物体坐标。
...
您会发现 CoVLA 数据集的结构非常复杂，层级较多。收集到的数据分成了多个 `.jsonl` 文件，并与原始视频片段（`.mp4`）一起存放在数据集中。

在 Zilliz Cloud 中，您可以使用 JSON 字段或者 Struct Array 字段来处理多层嵌套结构。当内层嵌套结构中包含向量字段时，Struct Array 是唯一选择。但是，Struct Array 字段内的 Struct 并不支持 JSON 或 Array 字段。为了将 CoVLA 数据集存入 Collection，您需要移除不必要的嵌套层级。
...
- `traffic_lights` 是一个 JSON 字段，包含当前帧中识别出的所有交通信号灯信号。

- `front_cars` 是另一个 Struct Array 字段，包含当前帧中识别出的所有前车信息。
...
- 初始化 front_cars Struct Schema
    ...
    虽然 front_car 对象并不包含向量，但是因为数据体积超过 JSON 字段的上限，您仍然需要将其作为 Struct Array 引入。
...
所有向量字段都需要索引。为 Struct Array 字段中的向量字段创建索引时，需要使用 `AUTOINDEX` 作为索引类型，并在 `MAX_SIM` 系列相似度类型中选择合适的类型来度量 EmbeddingList 之间的相似度。
...
Turing Motors 将 CoVLA 数据集分散到多个文件中，包括原始视频片段（`.mp4`）、自车状态（`states.jsonl`）、场景描述（`captions.jsonl`）、交通信号灯（`traffic_lights.jsonl`）以及前方车辆信息（`front_cars.jsonl`）。

您需要从这些数据中抽取每个视频片段的信息，并按照之前创建好的 Schema 格式重新组合，才能将其插入到 Collection 中。下方示例代码演示了如何合并指定视频片段的所有数据。
...
经过上述方式处理后，您就可以向 Collection 中插入这条视频片段的相关数据了。
