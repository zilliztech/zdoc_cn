...
如果在创建 Schema 时设置了 `enable_dynamic_fields=True`，Zilliz Cloud 允许您插入的数据中包含 Schema 中未定义的字段。但是这可能会使查询复杂度和管理难度增加，进而影响查询性能。关于动态字段的相关内容，可以参考[动态字段](./enable-dynamic-field)。
...
您也可以参考如下代码，查找 `varchar_field2` 字段为 `null` 的所有 Entity。
...
如果需要获取 `varchar_field1` 字段值为 `"Unknown"` 的所有 Entity，可以参考如下代码进行查询。需要注意的是，因为 `varchar_field1` 字段的默认值也为 `"Unknown"`，查询结果会包括该字段显式设置为 `"Unknown"` 的所有 Entity，以及该字段值为空的所有 Entity。
