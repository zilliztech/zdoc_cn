...
通过符号化和评分体系，文档由词袋向量表示，每个维度对应词表中的一个单词。只有文档中出现的单词才会在对应的向量维度上具有非 0 取值。将所有非 0 维度拼接起来，就形成了该文档的稀疏向量表示。您可以使用以下两种方式生成稀疏向量：

- **传统统计方法**：例如词频-逆文档频率（[TF-IDF](https://en.wikipedia.org/wiki/Tf%E2%80%93idf)）和最佳匹配 25（[BM25](https://en.wikipedia.org/wiki/Okapi_BM25)）等。这些方法基于词汇在语料库中的频率和重要性为词汇赋予权重，通过计算简单统计量作为每个维度的评分。每个维度代表一个词元。Zilliz Cloud 内置基于 BM25 算法的全文搜索功能，可自动将文本转换为稀疏向量，无需人工预处理。该方案特别适用于强调精确匹配和搜索精度的关键词检索场景。更多详情，请参阅[全文搜索](./full-text-search)。

- **神经稀疏向量嵌入模型**：基于大量数据训练、可以生成稀疏向量表示的模型。通常是 Transformer 架构的深度学习模型，能够根据语义上下文扩展并权衡待评估的词句。Zilliz Cloud 也支持使用诸如 [SPLADE](https://arxiv.org/abs/2109.10086) 等外部生成式稀疏向量模型。更多详情，请参阅 [Embeddings](https://milvus.io/docs/embeddings.md#Embedding-Overview)。

数据向量化后，可以存储在 Zilliz Cloud 中进行管理和向量检索。下图展示了基本流程。
...
在创建 Collection 之前，需要定义 Collection Schema。这包括添加字段，以及定义将文本转换为对应稀疏向量表示的派生方法。

### 添加字段\{#add-fields}

要在 Zilliz Cloud 中使用稀疏向量，首先需要在创建 Collection 时定义用于存储稀疏向量的字段。这个过程包括：

1. 将 `datatype` 设置为支持的稀疏向量数据类型，即 `SPARSE_FLOAT_VECTOR`。该字段的值可以由您提供，也可以由指定的 VARCHAR 字段自动转换而来。

1. 通常建议您将原始文本和向量字段都存入 Collection 中。您可以使用 VARCHAR 字段来存放原始文本。
...
以上示例中，我们添加了三个字段。它们分别是：

- `pk`：该字段用于存放 VARCHAR 类型的主键，其值为自动生成的文本，最大长度不超过 100 字节。
...
如果希望 Zilliz Cloud 在插入数据时将指定的文本转换为稀疏向量，您还需要创建 Function。关于 Function 的更多内容，可以参考[全文搜索](./full-text-search)。

</Admonition>

## 为稀疏向量创建索引\{#set-index-params-for-vector-field}

为稀疏向量创建索引的过程和[稠密向量](./use-dense-vector)类似，但指定的索引类型（`index_type`）、距离度量类型（`metric_type`）和索引参数（`params`）有所不同。
...
上述示例使用的索引类型为 `SPARSE_INVERTED_INDEX`，度量类型为 `BM25`。更多详细内容，可以参考如下内容：

- [相似度类型](./search-metrics-explained)：不同类型字段支持的度量类型。
...
插入数据时，您需要为所有在 Schema 中定义的字段提供相应的值，但自动生成的字段除外（例如因开启 AutoID 而自动生成的主键等）。如果您使用内置的 BM25 功能自动生成稀疏向量字段，在插入数据时也不需要提供该字段的取值。
