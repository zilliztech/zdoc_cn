...
`decompounder` 过滤器是 Zilliz Cloud 中的自定义过滤器，通过在过滤器配置中设置 `"type": "decompounder"` 并将 `word_list` 设置为您需要的值来指定。
...
`decompounder` 过滤器作用于分词器生成的词项，因此必须与分词器结合使用。有关 Zilliz Cloud 中可用的分词器列表，请参阅[分词器参考](./undefined)。
