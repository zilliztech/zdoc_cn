...
在验证 Analyzer 配置后，您就可以参考如下示例将 Analyzer 添加到目标字段中。
...
Collection 创建后，Analyzer 配置不可修改。如需更改 Analyzer 配置，请创建新的 Collection 并[迁移](./migrate-between-clusters)数据。

</Admonition>

## 后续步骤\{#whats-next}

在配置 Analyzer 时，建议您参考如下文章，了解如何针对您的数据集选择合适的 Analyzer 配置。

-  [最佳实践](./choose-the-right-analyzer-for-your-use-case)

配置 Analyzer 后，您就可以使用 Zilliz Cloud 提供的文本检索能力。更多内容，请参阅：
