...
向多语言 Collection 添加文档时，每个文档都必须同时包含文本内容和语言标识符：
...
使用多语言 Analyzer 进行搜索时，`search_params` 包含关键配置：

- `metric_type="BM25"`：必须与索引配置一致。

- `analyzer_name="english"`：指定应用于查询文本的 Analyzer。此设置与存储文档时使用的 Analyzer 相互独立。
...
此示例演示如何切换到中文 Analyzer（使用其别名 `"cn"`）来处理不同的查询文本。其他参数保持不变，但查询文本会按照中文特有的分词规则进行处理。
