...
<table>
   ...
   <tr>
     ...
     <td><ul><li><p>输入: <code>["a", "very", "extraordinarily"]</code></p></li><li><p>输出: <code>[['a'], ['very'], []]</code>（当 <strong>max=10</strong> 时）</p></li></ul></td>
   </tr>
   <tr>
     <td><p><a href="./regex-filter"><code>regex</code></a></p></td>
     <td><p>基于自定义模式的过滤</p></td>
     <td><p>特定领域的 token 需求</p></td>
     <td><ul><li><p>输入: <code>["test123", "prod456"]</code></p></li><li><p>输出: <code>[[], ['prod456']]</code>（当 <strong>expr="^prod"</strong> 时）</p></li></ul></td>
   </tr>
</table>
...
当处理跨多种语言或文字体系且不可预测的内容时，建议使用 `icu` analyzer。该 Analyzer 具备 Unicode 识别能力，能有效处理混合文字和符号。
