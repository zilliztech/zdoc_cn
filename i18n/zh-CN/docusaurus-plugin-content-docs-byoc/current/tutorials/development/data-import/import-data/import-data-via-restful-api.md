...
sidebar_label: "通过 RESTful API 导入"
beta: FALSE
added_since: FALSE
last_modified: FALSE
...
Zilliz Cloud 允许您将数据从任意对象存储导入到任意 Zilliz Cloud 集群，无需考虑托管集群的云服务商。例如，您可以将存储在阿里云上的数据导入到部署在腾讯云上的集群。

但是，Zilliz Cloud 仍建议您从与目标集群位于相同云服务商、相同云地域的对象存储桶中导入数据，以获得低时延、高稳定性的数据操作体验。
...
为了成功导入数据，请确保目标 Collection 中正在运行或待运行的导入任务不超过 10 个。

</Admonition>

一旦获取到文件路径和存储桶凭据，请按以下方式调用 API：
...
在上述代码中，`${CLOUD_REGION_ID}` 代表您集群所在云地域的 ID，`${TOKEN}` 是用于授权 API 请求的集群 API 密钥，`${CLUSTER_ID}` 是您的集群 ID。调用 API 时，请确保将这些占位符替换为实际值。您可以从集群的公共访问端点获取 `CLOUD_REGION_ID` 和 `CLUSTER_ID`。例如，在公共访问端点 **https://in03-3bf3c31f4248e22.api.ali-cn-hangzhou.cloud.zilliz.com.cn** 中，`CLOUD_REGION_ID` 是 **ali-cn-hangzhou**，`CLUSTER_ID` 是 **in03-3bf3c31f4248e22**。
...
有关更多信息，请参见[导入](https://docs.zilliz.com.cn/reference/restful/import)和[查看导入进度](https://docs.zilliz.com.cn/reference/restful/get-import-progress)。
...
您也可以调用 RESTful API 获取[当前导入任务的进度](https://docs.zilliz.com.cn/reference/restful/get-import-job-progress-v2)，并[列出所有导入任务](https://docs.zilliz.com.cn/reference/restful/list-import-jobs-v2)以获取更多信息。作为替代方案，您也可以前往 Zilliz Cloud 控制台上的[任务中心](./job-center)查看结果和作业详情。
