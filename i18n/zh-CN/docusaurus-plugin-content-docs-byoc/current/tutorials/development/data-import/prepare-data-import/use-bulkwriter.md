...
notebook: FALSE
description: "如果您的数据格式不满足支持的数据格式中列出的各项要求，您可以使用 PyMilvus 和 Milvus Java SDK 自带的 BulkWriter 对您的数据进行转换。 | BYOC"
type: origin
...
- **LocalBulkWriter**
    ...
    **LocalBulkWriter** 将原始数据按行添加到缓存中，然后将缓存中的数据存入一个指定格式的本地文件中。
    ...
    如需导入其它格式的文件，需要先将它们上传到您的对象存储桶中。建议您使用与您的集群同地域的对象存储桶。
