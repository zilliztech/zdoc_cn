...
建议您[使用 BulkWriter](./use-bulkwriter) 工具将原始数据转换为 JSON 文件。下图演示了原始数据如何映射到 JSON 文件中。
...
- **大小写**
    ...
    字典键名和 Collection 的字段名区分大小写。您需要确保待导入字典的键名与目标 Collection 的字段名完全对应。例如，如果目标 Collection 中有一个名为 **id** 的字段，那么待导入字典的键名也应为 **id**。使用 **ID** 或 **Id** 会导致报错。
...
如果您的文件体积较小，建议使用多路径或源文件夹方式一次性导入所有文件。Zilliz Cloud 针对这两种方式进行了内部优化，可以降低后续资源消耗。
...
当您有多个文件需要同时导入时，可以使用这种方式。导入时，需要将每个待导入 JSON 文件的路径放入一个子列表中，并将这些子列表放入一个外层列表中，如下方代码所示。
...
如果源文件夹中仅包含需要导入的所有文件，可以将该源文件夹的路径放入一个子列表中，然后将该子列表放入一个外层列表中，如下方代码所示。
...
如果您希望单独导入一个 JSON 文件，可以将该 JSON 文件的路径放入一个子列表中，然后将该子列表放入一个外层列表中，如下方代码所示。
...
请注意，一个合法的 JSON 文件中应有一个名为 **rows** 的根键，其值为一个字典列表，其中每个字典代表一个需要插入的 Entity 对象。

</Admonition>

| **导入方式** | **单次导入最大文件数** | **单个文件大小上限** | **单次导入总文件大小** |
| --- | --- | --- | --- |
