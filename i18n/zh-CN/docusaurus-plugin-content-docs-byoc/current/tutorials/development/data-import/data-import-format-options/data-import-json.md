...
notebook: FALSE
description: "JSON 文件是一种轻量级、对人和机器都友好的数据格式。由于其语言无关特性，并遵循类 C 语言程序员熟悉的规范，JSON 是一种非常理想的数据交换格式。 | BYOC"
type: origin
...
[JSON](https://www.json.org/json-en.html) 文件是一种轻量级、对人和机器都友好的数据格式。由于其语言无关特性，并遵循类 C 语言程序员熟悉的规范，JSON 是一种非常理想的数据交换格式。
...
- **大小写**
    ...
    字典键名和 Collection 的字段名区分大小写。您需要确保待导入字典的键名和目标 Collection 的字段名完全对应。例如，目标 Collection 中有个字段名为 **id**，那么待导入字典的键名也应该为 **id**。使用 **ID** 或 **Id** 会导致报错。
...
如果您的文件体积较小，建议使用多个路径或源文件夹的方式将所有文件一次性导入。Zilliz Cloud 针对这两种方式做了内部优化，可以降低后续资源消耗。
...
当您有多个文件需要同时导入时，可以使用这种方式。导入时，需要将每个需要导入的 JSON 文件的路径包含在一个子列表中，并将这些子列表包含在一个外层列表中，如下方代码所示。
...
建议您[使用 BulkWriter](./use-bulkwriter) 工具将原始数据转换成 JSON 文件。我们按照本文示意图中的 Schema 准备了一个示例数据供您参考。[单击此处](https://assets.zilliz.com/prepared_json_data.json)下载该示例文件。
