...
notebook: FALSE
description: "Apache Parquet 是一种开源的列式数据文件格式，用于优化数据存储和检索效率。它提供高性能的数据压缩和编码方案来管理复杂的批量数据，并支持多种编程语言和分析工具。 | BYOC"
type: origin
...
[Apache Parquet](https://parquet.apache.org/docs/overview/) 是一种开源的列式数据文件格式，用于优化数据存储和检索效率。它提供高性能的数据压缩和编码方案来管理复杂的批量数据，并支持多种编程语言和分析工具。

建议您[使用 BulkWriter](./use-bulkwriter) 工具将原始数据转换为 Parquet 文件。下图演示了原始数据如何映射到 Parquet 文件中。
...
- **是否启用 Dynamic Field**
    ...
    当目标 Collection 启用了 Dynamic Field，并且您希望导入 Schema 中未定义的字段时，可以在导入数据中包含一个名为 **&#36;meta** 的字段，并将所有未在 Schema 中定义的字段以键值对的形式存放到 **&#36;meta** 字段中。

- **大小写**

    字典键名和 Collection 的字段名区分大小写。您需要确保待导入字典的键名与目标 Collection 的字段名完全对应。例如，如果目标 Collection 中有一个字段名为 **id**，那么待导入字典的键名也应为 **id**。使用 **ID** 或 **Id** 会导致报错。
...
如果文件体积较小，建议使用多路径或源文件夹的方式将所有文件一次性导入。Zilliz Cloud 针对这两种方式进行了内部优化，可以降低后续资源消耗。
...
当有多个文件需要同时导入时，可以使用这种方式。导入时，需要将每个待导入 Parquet 文件的路径放入一个子列表中，并将这些子列表放入一个外层列表中，如下方代码所示。
...
如果源文件夹中仅包含需要导入的所有 Parquet 文件，您可以将该源文件夹的路径放入一个子列表中，然后将该子列表放入一个外层列表中，如下方代码所示。
...
如果希望单独导入一个 Parquet 文件，可以将该 Parquet 文件的路径放入一个子列表中，然后将该子列表放入一个外层列表中，如下方代码所示。
...
Zilliz Cloud 对导入 Parquet 格式文件设置了以下限制。
...
建议您[使用 BulkWriter](./use-bulkwriter) 工具将原始数据转换为 Parquet 文件。我们按照本文示意图中的 Schema 准备了一个示例数据供您参考。[单击此处](https://assets.zilliz.com/prepared_parquet_data.parquet)下载该示例文件。
