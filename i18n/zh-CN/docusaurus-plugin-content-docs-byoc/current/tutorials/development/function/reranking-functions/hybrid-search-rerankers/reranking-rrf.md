...
notebook: FALSE
description: "RRF（Reciprocal Rank Fusion，互反排名融合）Ranker 是 Zilliz Cloud 混合搜索中的一种重排序策略，它基于多个向量搜索路径中的排名位置而非原始相似度得分来平衡结果。就像体育锦标赛会考虑选手的排名而非个人统计数据一样，RRF Ranker 会根据每个项目在不同搜索路径中的排名高低来合并搜索结果，从而生成公平且均衡的最终排名。 | BYOC"
type: origin
...
RRF（Reciprocal Rank Fusion，互反排名融合）Ranker 是 Zilliz Cloud 混合搜索中的一种重排序策略，它基于多个向量搜索路径中的排名位置而非原始相似度得分来平衡结果。就像体育锦标赛会考虑选手的排名而非个人统计数据一样，RRF Ranker 会根据每个项目在不同搜索路径中的排名高低来合并搜索结果，从而生成公平且均衡的最终排名。
...
| --- | --- | --- |
| 同等重要性的多模态搜索 | 图文搜索，两种模态同等重要 | 平衡结果，无需进行任意权重分配 |
| 集成向量搜索 | 结合不同嵌入模型的结果 | 以民主方式合并排名，不偏向任何特定模型的评分分布 |
| 跨语言搜索 | 跨多种语言查找文档 | 公平地对结果进行排名，不受特定语言嵌入特征影响 |
...
使用 RRF Ranker 策略时，需要配置参数 `k`。它是一个平滑参数，可以有效改变全文搜索与向量搜索的相对权重。该参数默认值为 60，可在 (0, 16384) 范围内调整。取值应为浮点数，建议值在 [10, 100] 之间。虽然 `k=60` 是常见选择，但最优 `k` 值可能因具体应用和数据集而异。建议根据具体用例测试并调整此参数，以获得最佳性能。
...
| `params.k` | 否 | 平滑参数，用于控制文档排名的影响；较高的 `k` 会降低对高排名的敏感度。范围：(0, 16384)；默认值：`60`。<br/>详情请参阅 [RRF Ranker 的工作机制](./reranking-rrf#mechanism-of-rrf-ranker)。 | `100` |
