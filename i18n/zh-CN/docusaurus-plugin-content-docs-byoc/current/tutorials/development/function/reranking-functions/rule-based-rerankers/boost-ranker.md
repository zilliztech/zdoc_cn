...
当搜索请求包含 Boost Ranker 函数时，Zilliz Cloud 使用该函数内的可选过滤条件在搜索结果候选集中查找匹配项，并通过应用指定的权重来提升这些匹配项的得分，从而在最终结果中提升或降低匹配 Entity 的排名。
...
以下示例展示了在单向量搜索中使用 Boost Ranker 的情况，该搜索需要返回前五个最相关的 Entity，并为具有摘要文档类型的 Entity 的得分添加权重。

1. **分 Segment 收集候选搜索结果。**
...
1. **根据加权得分汇总所有 Segment 的候选项，以确定最终结果。**
...
<table>
   ...
   <tr>
     ...
     <td><p>是</p></td>
     <td><p>要应用该函数的 vector 字段列表（对于 RRF Ranker 必须为空）</p></td>
     <td><p><code>[]</code></p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>是</p></td>
     <td><p>指定将与原始搜索结果中任何匹配 Entity 的得分相乘的权重。</p><p>该值应为浮点数。</p><ul><li><p>为强调匹配 Entity 的重要性，将其设置为能提高分数的值。</p></li><li><p>若要降低匹配 Entity 的排名，可将此参数赋值为能降低其得分的值。</p></li></ul></td>
     <td><p><code>1</code></p></td>
   </tr>
   <tr>
     <td><p><code>params.filter</code></p></td>
     <td><p>否</p></td>
     <td><p>指定用于在搜索结果 Entity 中匹配 Entity 的过滤表达式。它可以是<a href="./filtering-overview">过滤表达式概览</a>中提到的任何有效基本过滤表达式。</p><p><strong>注意</strong>：仅使用基本运算符，例如 <code>==</code>、<code>&gt;</code> 或 <code>&lt;</code>。使用高级运算符，例如 <code>text_match</code> 或 <code>phrase_match</code>，将降低搜索性能。</p></td>
     <td><p><code>"doctype == 'abstract'"</code></p></td>
   </tr>
   <tr>
     <td><p><code>params.random_score</code></p></td>
     <td><p>否</p></td>
     <td><p>指定一个随机函数，该函数会随机生成一个介于 <code>0</code> 和 <code>1</code> 之间的值。它有以下两个可选参数：</p><ul><li><p><code>seed</code>（数字）指定用于启动伪随机数生成器（PRNG）的初始值。</p></li><li><p><code>field</code>（字符串）指定一个字段的名称，该字段的值将用作生成随机数的随机因子。具有唯一值的字段就足够了。</p><p>建议你同时设置 <code>seed</code> 和 <code>field</code>，以通过使用相同的种子和字段值来确保各次生成之间的一致性。</p></li></ul></td>
