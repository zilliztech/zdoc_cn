...
<table>
   ...
   <tr>
     <td><p>战略性内容降权</p></td>
     <td><ul><li><p>在不完全移除低库存商品的情况下降低其显著程度</p></li><li><p>在不进行审查的情况下降低包含潜在令人反感词汇的内容的排名</p></li><li><p>在保留旧归档以供技术搜索访问的同时对其进行降级处理</p></li><li><p>在市场搜索中巧妙降低竞品的可见度</p></li><li><p>降低具有低质量迹象（如格式问题、篇幅较短等）的内容的相关性</p></li></ul></td>
   </tr>
</table>
...
1. **应用在 Boost Ranker 中指定的权重**（`权重=0.5`）。
    ...
    | ID | 文档类型 | 得分 | 加权分数<br/>(=得分×权重) | 排名 | Segment |
    | --- | --- | --- | --- | --- | --- |
...
1. **根据加权分数汇总所有 Segment 的候选项，以确定最终结果。**
...
<table>
   ...
   <tr>
     ...
     <td><p>否</p></td>
     <td><p>指定用于在搜索结果实体中匹配实体的过滤表达式。它可以是<a href="./filtering-overview">过滤概述</a>中提到的任何有效基本过滤表达式。</p><p><strong>注意</strong>：仅使用基本运算符，例如 <code>==</code>、<code>&gt;</code> 或 <code>&lt;</code>。使用高级运算符，例如 <code>text_match</code> 或 <code>phrase_match</code>，将降低搜索性能。</p></td>
     <td><p><code>"doctype == 'abstract'"</code></p></td>
   </tr>
   <tr>
     <td><p><code>params.random_score</code></p></td>
     <td><p>否</p></td>
     <td><p>指定一个随机函数，该函数会随机生成一个介于 <code>0</code> 和 <code>1</code> 之间的值。它有以下两个可选参数：</p><ul><li><p><code>seed</code>（数字）指定用于启动伪随机数生成器（PRNG）的初始值。</p></li><li><p><code>field</code>（字符串）指定一个字段的名称，该字段的值将用作生成随机数的随机因子。具有唯一值的字段就足够了。</p><p>建议你同时设置 <code>seed</code> 和 <code>field</code>，以通过使用相同的种子和字段值来确保各代之间的一致性。</p></li></ul></td>
