...
notebook: FALSE
description: "指数衰减会使搜索结果呈现出初始阶段急剧下降，随后是长尾的特征。就像突发新闻周期一样，相关性起初迅速减弱，但有些报道随着时间的推移仍保持重要性，指数衰减对刚刚超出理想范围的项目大幅罚分，同时仍让距离较远的项目可以被发现。当你希望高度优先考虑接近程度或时效性，但又不想完全排除距离较远的选项时，这种方法是理想的选择。 | BYOC"
type: origin
...
指数衰减会使搜索结果呈现出初始阶段急剧下降，随后是长尾的特征。就像突发新闻周期一样，相关性起初迅速减弱，但有些报道随着时间的推移仍保持重要性，指数衰减对刚刚超出理想范围的项目大幅罚分，同时仍让距离较远的项目可以被发现。当你希望高度优先考虑接近程度或时效性，但又不想完全排除距离较远的选项时，这种方法是理想的选择。
...
指数衰减独特地“前置”罚分，在衰减早期就开始大幅罚分，但仍保持最小但非零的相关性得分长尾。
...
- 如果较旧或较远的项目具有极高的相关性，那么它们仍应是可被发现的
...
- `origin`（当前时间）：又称为原点，表示当前时刻，相关性达到最大值（1.0）。

- `offset`（3小时）：又称为偏移量，表示“突发新闻窗口”——所有在过去3小时内发布的报道都保持完整的相关性得分（1.0），确保非常近期的新闻不会因微小的时间差异而被不必要地扣分。

- `decay` (0.5)：又称为衰减值，表示在关注范围边缘处的得分——此参数控制得分随时间衰减的剧烈程度。
...
1. 乘以$\lambda$，该值由您的 scale 和 decay 参数计算得出。

1. 取指数，它会给出一个介于0和1之间的值：$\exp(\lambda \cdot value)$。

该$\lambda$计算将您的 scale 和 decay 参数转换为指数函数的速率参数。更负的$\lambda$会导致更陡峭的初始下降。
...
在使用 Decay Ranker 之前，你必须首先创建一个包含适当数字类型字段（如时间戳、距离等）的 Collection，这些字段将用于衰减计算。有关包括 Collection 设置、 Schema 定义和数据插入的完整工作示例，请参考[教程：实现基于时间的搜索结果重排](./tutorial-implement-time-based-ranking)。
...
**时间单位一致性**：使用基于时间的衰减时，确保 `origin`、`scale` 和 `offset` 参数使用与您的 Collection 中的数据相同的时间单位。如果您的 Collection 中的数据以秒为单位存储时间戳，则所有参数都使用秒。如果使用毫秒，则所有参数都使用毫秒。
