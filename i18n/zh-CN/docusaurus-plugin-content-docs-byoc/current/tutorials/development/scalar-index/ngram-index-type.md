...
notebook: FALSE
description: "在 Zilliz Cloud 中，NGRAM 索引用于加速对 VARCHAR 字段或 JSON 字段中指定路径的 LIKE 查询。在建立索引之前，Zilliz Cloud 会将文本拆分为固定长度 n 的重叠子串（n-gram）。例如，当 `n = 3` 时，单词 `\"Milvus\"` 会被拆分为以下 3-gram：`\"Mil\"`, `\"ilv\"`, `\"lvu\"`, `\"vus\"`。这些 n-gram 随后会存储在倒排索引中，每个 gram 都映射到包含它的文档 ID。在查询时，该索引使 Zilliz Cloud 能快速缩小候选范围，从而显著加速查询执行。 | BYOC"
type: origin
...
当你需要快速进行 **前缀、后缀、中缀或通配符过滤** 时，可使用 NGRAM 索引：
...
1. **将文本分解为 n-gram**
    ...
    **示例**：当 `min_gram=2, max_gram=3` 时，字符串 `"AI database"` 会被分解为：
    ...
    - 保留大小写，**区分大小写**（如 `"Database"` 与 `"database"` 会生成不同 n-gram）

    </Admonition>

1. **建立倒排索引**：构建倒排索引，将每个 n-gram 映射到包含它的文档 ID 列表。

    例如，若 2-gram `"AI"` 出现在文档 1、5、6、8、9 中，则索引记录为：`{"AI": [1,5,6,8,9]}`。
...
1. **提取查询词**：从 LIKE 表达式中提取不带通配符的连续子串（如 `"%database%"` → `"database"`）

1. **分解查询词**：根据查询词长度 L 与 [min_gram, max_gram] 进行拆分：

    - L < min_gram → 索引不可用，回退到全表扫描

    - min_gram ≤ L ≤ max_gram → 将查询词整体视为一个 n-gram
...
### 示例 1：在 VARCHAR 字段上创建\{#example-1-create-on-a-varchar-field}
...
此配置会为 `text` 字段中的每个字符串生成 2-gram 和 3-gram，并将其存储到倒排索引中。

### 示例 2：在 JSON 路径上创建\{#example-2-create-on-a-json-field}
...
- Zilliz Cloud 会生成长度为 2–4 的子串并进行存储
...
如果您的集群与 Milvus v2.6.x 兼容，则可以删除标量字段上的索引，无需对 Collection 执行 Release 操作。
...
- **选择合适的 min_gram 和 max_gram**
    ...
    - 将 min_gram 设置为用户可能输入的最短字面量长度

    - 将 max_gram 设置为常见有效子串的典型长度（越大 → 过滤更精确，但索引更大）
