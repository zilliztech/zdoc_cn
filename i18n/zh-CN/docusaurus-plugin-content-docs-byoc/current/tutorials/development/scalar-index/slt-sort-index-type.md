...
notebook: FALSE
description: "在 Zilliz Cloud 中使用 `STL_SORT` 索引，可以将如 `INT8` 与 `INT16` 这一类数值类型字段、`VARCHAR` 字段或 `TIMESTAMPTZ` 字段中的值按照既定方式排列，从而提升在上述字段中进行查询时的性能。 | BYOC"
type: origin
...
在 Zilliz Cloud 中使用 `STL_SORT` 索引，可以将如 `INT8` 与 `INT16` 这一类数值类型字段、`VARCHAR` 字段或 `TIMESTAMPTZ` 字段中的值按照既定方式排列，从而提升在上述字段中进行查询时的性能。
...
在使用 `STL_SORT` 索引时，Zilliz Cloud 按照如下步骤创建索引并加速查询：
...
1. 加速查询
    ...
    1. 在查询时，Zilliz Cloud 在上述已排序数组中使用 C++ 的 [std:lower_bound](https://en.cppreference.com/w/cpp/algorithm/lower_bound.html) 和 [std::upper_bound](https://en.cppreference.com/w/cpp/algorithm/upper_bound.html) 方法进行二分查询。

    1. 对于大小比较场景，Zilliz Cloud 能够快速找到所有匹配的值。

    1. 对于范围比较场景，Zilliz Cloud 则能迅速定位到范围的起止位置，并返回其间的所有值。

    1. 在该字段中包含匹配值的 Entity 主键将被传递到查询执行器，用于组装最终查询结果。

上述步骤使得查询复杂度由 **O(n)** 降低到 **O(logn +m)**，其中 m 是匹配 Entity 的数量。
