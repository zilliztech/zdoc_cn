...
notebook: FALSE
description: "在 Zilliz Cloud 中使用 `STL_SORT` 索引，可以将如 `INT8` 与 `INT16` 这一类数值类字段、`VARCHAR` 字段或 `TIMESTAMPTZ` 字段中的值按照既定方式排列，从而提升在上述字段中进行查询时的性能。 | BYOC"
type: origin
...
在 Zilliz Cloud 中使用 `STL_SORT` 索引，可以将如 `INT8` 与 `INT16` 这一类数值类字段、`VARCHAR` 字段或 `TIMESTAMPTZ` 字段中的值按照既定方式排列，从而提升在上述字段中进行查询时的性能。

如果你经常需要执行如下查询，可以考虑使用 `STL_SORT` 索引：
...
- `TIMESTAMPTZ` 类型。更多详情，可参考 [TIMESTAMPTZ 类型](./use-timestamptz-field)。

## 工作原理\{#how-it-works}

使用 `STL_SORT` 索引时，Zilliz Cloud 按照如下步骤创建索引并加速查询：
...
1. 创建索引
    ...
    1. 每个值都会与其所在 Entity 的主键一起存入一个已排序数组中。

1. 加速查询

    1. 查询时，Zilliz Cloud 会在上述已排序数组中使用 C++ 的 [std:lower_bound](https://en.cppreference.com/w/cpp/algorithm/lower_bound.html) 和 [std::upper_bound](https://en.cppreference.com/w/cpp/algorithm/upper_bound.html) 方法进行二分查询。

    1. 对于大小比较的场景，Zilliz Cloud 能够快速找到所有匹配的值。

    1. 对于范围比较的场景，Zilliz Cloud 能够迅速定位到范围的起止位置，并返回其间的所有值。

    1. 在该字段中包含匹配值的 Entity 主键将被传递给查询执行器，用于组装最终查询结果。

上述步骤使查询复杂度从 **O(n)** 降低到 **O(logn +m)**，其中 m 指匹配 Entity 的数量。

## 创建 STL_SORT 索引\{#create-an-stl_sort-index}

你可以在数值、`VARCHAR` 或 `TIMESTAMPTZ` 字段上直接使用 `STL_SORT` 索引，无需额外参数。如下示例演示如何在 `TIMESTAMPTZ` 字段上使用 `STL_SORT` 索引。
...
你也可以使用 `drop_index()` 从 Collection 中删除指定字段上的索引。

<Admonition type="info" icon="📘" title="说明">

如果你的集群与 Milvus v2.6.x 兼容，你可以删除标量字段上的索引，无需对 Collection 执行 Release 操作。
