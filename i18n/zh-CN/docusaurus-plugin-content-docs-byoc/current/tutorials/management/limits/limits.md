...
计算单元（CU）是指用于提供向量检索、分析服务的一组硬件资源。CU 提供了并行数据处理的基础计算资源，不同类型的 CU 结合了各异的 CPU、内存和存储配置。CU 的概念仅适用于 Dedicated 集群。
...
如需创建超过 2048 CU 规格的集群，请[联系我们](https://zilliz.com.cn/contact-sales?firstname=xushuang&lastname=hu&company=Zilliz&name=Zilliz&email=xushuang.hu@zilliz.com&fullname=hu%20xushuang&phone=--&country=)。
...
根据您的集群使用的 CU 数量不同，每 CU 支持创建最多 256 个 Collection 或 1,024 个 Partition。其中，每个 Collection 最多可创建 1,024 个 Partition。您可以参考如下公式计算集群中 Collection 和 Partition 的数量上限。

![QJ1jwpa8YhQegqbrAYWcJEOUnWf](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/QJ1jwpa8YhQegqbrAYWcJEOUnWf.png)

- 集群中 Collection 的数量上限应取 256 与集群 CU 数量之积和 16,384 之间的较小值。

- 集群中所有 Collection 的 Partition 总数量应取 1,024 与集群 CU 数量之和和 65,536 之间的较小值。

- 上述条件需同时满足。

### 兼容 Milvus v2.5.x 的集群\{#clusters-compatible-with-milvus-v25x}

根据您的集群使用的 CU 数量不同，每 CU 支持创建最多 1,024 个 Collection 或 4,096 个 Partition。其中，每个 Collection 最多可创建 1,024 个 Partition。您可以参考如下公式计算集群中 Collection 和 Partition 的数量上限。

![RBu3wbDwihqeFgbFZwQcIpXNnUf](https://zdoc-images.oss-cn-hangzhou.aliyuncs.com/RBu3wbDwihqeFgbFZwQcIpXNnUf.png)

- 集群中 Collection 的数量上限应取 1,024 与集群 CU 数量之积和 16,384 之间的较小值。

- 集群中所有 Collection 的 Partition 总数量应取 4,096 与集群 CU 数量之积和 65,536 之间的较小值。
...
- 某些字段（如 VarChar 或 JSON）使用的内存可能超出预期，导致集群空间耗尽。
...
| **内容** | **最大数量** | **描述** |
| --- | --- | --- |
| Private Endpoint | 10 | 每个项目最多可创建 10 个 Private Endpoint。 |
