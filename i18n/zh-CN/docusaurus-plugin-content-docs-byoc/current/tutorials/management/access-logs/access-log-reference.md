...
notebook: FALSE
description: "访问日志以 JSON Lines 格式传输——每行一个 JSON 对象。每行是一个独立的 JSON 对象，代表一次操作。以下示例展示了一个 Search 操作，为便于阅读进行了格式化： | BYOC"
type: origin
...
访问日志以 [JSON Lines](https://jsonlines.org/) 格式传输——每行一个 JSON 对象。每行是一个独立的 JSON 对象，代表一次操作。以下示例展示了一个 Search 操作，为便于阅读进行了格式化：
...
| `params.input_params` | 否 | object | 操作的输入参数（搜索参数、Offset、Topk 等）。HybridSearch 中包含带 `sub_0.*` 前缀的子请求参数和 `strategy`。 | `{"topk": "10", "offset": "0"}` |
