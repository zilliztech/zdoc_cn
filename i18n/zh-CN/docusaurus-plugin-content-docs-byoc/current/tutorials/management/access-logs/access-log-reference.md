...
notebook: FALSE
description: "访问日志以 JSON Lines 格式传输——每行一个 JSON 对象。每行都是一个独立的 JSON 对象，代表一次操作。以下示例展示了一个 Search 操作，为便于阅读已进行格式化： | BYOC"
type: origin
...
访问日志以 [JSON Lines](https://jsonlines.org/) 格式传输——每行一个 JSON 对象。每行都是一个独立的 JSON 对象，代表一次操作。以下示例展示了一个 Search 操作，为便于阅读已进行格式化：
...
| **字段** | **必填** | **类型** | **描述** | **示例** |
| --- | --- | --- | --- | --- |
| `action` | 是 | string | 操作名称。请参阅[支持的操作](./access-log-reference#supported-actions)。 | `"Search"` |
...
| `params.input_params` | 否 | object | 操作的输入参数（搜索参数、Offset、Topk 等）。HybridSearch 中包含带有 `sub_0.*` 前缀的子请求参数和 `strategy`。 | `{"topk": "10", "offset": "0"}` |
...
| `params.partition` | 否 | string | 目标 Partition（如果已指定）。未指定时为 `null`。 | `null` |
| `params.result_num` | 否 | int | 操作实际返回的结果数量。 | `10` |
| `params.result_pks` | 否 | array | 查询结果中的主键。出现在 Search、HybridSearch 和 Query 操作中，且需要配置输出参数包含该字段。 | `[55, 19, 18, 10]` |
