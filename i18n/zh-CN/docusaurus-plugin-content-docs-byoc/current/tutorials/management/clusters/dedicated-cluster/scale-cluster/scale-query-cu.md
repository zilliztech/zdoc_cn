...
- **备份限制**：动态和定时扩缩容设置不会包含在[备份](./undefined)中。恢复集群后需要重新手动配置。

## 手动扩缩容\{#manual-scaling}

您可以通过 Zilliz Cloud 控制台或 RESTful API 手动扩展或缩减集群的 Query CU 数量。

以下演示展示了如何在 Zilliz Cloud Web 控制台中手动扩容或缩容集群。
...
以下示例将现有集群扩展为 2  CU。详情请见[修改集群配置](https://docs.zilliz.com.cn/reference/restful/modify-cluster-v2)。
...
您也可以使用 RESTful API 启用定时扩缩容。详情请见[修改集群配置](https://docs.zilliz.com.cn/reference/restful/modify-cluster-v2)。
...
| 变量名称 | 描述 |
| --- | --- |
| 目标 Query CU 数量 | 系统计划将集群扩缩容至的新 Query CU 数量 |
| 当前 Query CU 数量 | 集群当前的 Query CU 数量 |
| 当前指标值 | 当前测得的 Query CU 加载容量指标值 |
| 目标指标值 | 扩缩容后期望达到的 CU 加载容量值，固定为 70 |
...
以下演示展示了如何在 Zilliz Cloud 控制台中配置动态扩缩容。
