...
notebook: FALSE
description: "在 Zilliz Cloud 中，Query CU 是一组用于处理索引和搜索请求的硬件资源。您可以将 Query CU 理解为一个全托管的物理节点，用于运行您的查询服务。Replica 是集群级别的拷贝，包含相同的集群资源与数据。Query CU 主要决定集群容量与计算资源，而 Replica 为查询服务提供额外的并行能力。 | BYOC"
type: origin
