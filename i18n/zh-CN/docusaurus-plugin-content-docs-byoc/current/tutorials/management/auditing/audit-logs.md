...
在 BYOC 项目中，VDB 审计日志直接写入您的数据面本地对象存储中指定路径下，确保您的数据不会离开您的基础设施。如需启用或配置审计日志，请[联系我们](https://support.zilliz.com.cn/hc/zh-cn)。

</Admonition>

## 概述\{#overview}

审计日志记录会跟踪数据面上的各种操作，包括：
...
所有审计日志都将直接推送到您在部署数据面时指定的对象存储桶内。

如需将审计日志导出到您的日志系统进行深入分析，请[联系我们](https://support.zilliz.com.cn/hc/zh-cn/requests/new)。
