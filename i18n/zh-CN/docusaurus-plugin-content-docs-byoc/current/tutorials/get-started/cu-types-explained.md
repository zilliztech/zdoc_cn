...
| 向量维度 | 性能型集群（每 Query CU可容纳最大向量数） | 容量型集群（每 Query CU 可容纳最大向量数） | 分层存储型集群（每 Query CU 可容纳最大向量数） |
| --- | --- | --- | --- |
...
| top_k 值 | 性能型集群的 QPS（768 维度，150万向量） | 容量型集群的 QPS（768 维度，500 万向量） |
| --- | --- | --- |
| 10 | 520 | 100 |
| 100 | 440 | 80 |
| 250 | 270 | 60 |
...
| top_k | 性能型集群的时延（768 维度，150万向量） | 容量型集群的时延（768 维度，500 万向量） |
| --- | --- | --- |
| 10 | < 10 ms | < 50 ms |
| 100 | < 10 ms | < 50 ms |
| 250 | < 10 ms | < 50 ms |
