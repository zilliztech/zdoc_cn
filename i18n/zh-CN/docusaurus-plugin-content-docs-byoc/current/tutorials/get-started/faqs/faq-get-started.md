...
beta: FALSE
notebook: FALSE
description: "本文列举了快速上手 Zilliz Cloud 时可能遇到的常见问题及对应解决方法。 | BYOC"
type: origin
...
您可以使用向量数据库性能对比工具 [VectorDBBench](https://zilliz.com/vector-database-benchmark-tool) 来衡量和比较 Zilliz Cloud 与其他主流向量数据库和云服务之间的性能差异。
...
通常，对于包含 500 万个 768 维向量的容量型集群，其搜索延迟约为 100 毫秒。对于相同数据量和维度的性能型集群，搜索延迟约为 20 毫秒。但请注意，实际搜索延迟可能因您的工作负载和部署环境而有所不同。

### Zilliz Cloud 支持哪些索引类型？\{#which-type-of-index-is-supported-by-zilliz-cloud}

目前，Zilliz Cloud 仅支持 AUTOINDEX。AUTOINDEX 是 Zilliz Cloud 自研索引，可增强搜索性能。对于包含 150 万个 768 维向量的性能型集群，QPS 可达到几百，延迟低于 100 毫秒。对于包含 500 万个 768 维向量的容量型集群，QPS 可达到 50，延迟超过 200 毫秒。有关更多详细信息，请参见 [AUTOINDEX](./autoindex-explained)。
