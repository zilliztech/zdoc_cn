...
[Zilliz Cloud cluster](/docs/manage-cluster) 是运行在 Zilliz Cloud 上的 Milvus 实例。Zilliz Cloud 将其 clusters 分类为多种产品形态，包括 **Free**、**Serverless**、**Dedicated (Standard)、Dedicated (Enterprise)** 和 **Bring Your Own Cloud (BYOC)**。有关这些产品形态的详细信息，请参阅 [详细套餐对比](/docs/select-zilliz-cloud-service-plans)。
