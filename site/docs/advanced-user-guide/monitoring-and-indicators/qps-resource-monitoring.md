...
为了及时调整 Zilliz Cloud 集群的 QPS，建议启用 QPS 资源监控。

## 设置告警条件

要在项目中设置集群的 QPS 告警条件，请执行以下步骤：

1. 在项目左侧导航栏中选择**监控**。

1. 打开**监控设置**页签。

1. 定位到**资源监控**区域下的第二行条件。
  默认情况下，当 QPS 超过 90％ 且持续时间超过 10 分钟时，我们将向您发送告警通知。

1. 单击操作列的**更多**按钮，选择**编辑**。
  在提示的对话框中调整 QPS 和持续时长，然后单击**保存**。
...
- [查询延时监控](https://zilliverse.feishu.cn/wiki/QBaRweeRriuYskkMWxhcLOkyneI)
