...
# 初始化 MilvusClient 实例
# 使用您自己的 uri 和用户名密码替换以下参数值
client = MilvusClient(
    # 请在 Zilliz Cloud 控制台获取集群访问地址。
    uri="<CLUSTER-ENDPOINT>", 
    # 格式示例，请使用半角冒号将访问集群的用户名和密码隔开。
    # 注意：此处并非您的云平台登录帐号。
...
#     ], 
#     'aliases': [],
#     'collection_id': 442614933171474466,
#     'consistency_level': 2,
#     'properties': [],
#     'num_partitions': 1,
...
- 插入多个 Entity
    ...
    # 此步骤非必须。
    # Zilliz Cloud 会在新插入的数据到达一定规模时自动发起写入（Flush）操作。
    # 如果希望立即查询刚才插入的数据，可以考虑使用如下命令让 Zilliz Cloud 立即执行写入操作。
    # 此处出于演示目的，选择手动执行该操作。
