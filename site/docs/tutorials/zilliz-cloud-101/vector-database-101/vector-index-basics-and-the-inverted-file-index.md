---
slug: /vector-index-basics-and-the-inverted-file-index
sidebar_position: 3
---
...
## Milvus **中的向量索引** {#vector-index-in-milvus}

Milvus 使用 [Facebook AI Similarity Search（FAISS）](https://github.com/facebookresearch/faiss)作为关键的索引构建库，除此之外还使用了 [Hnswlib](https://github.com/nmslib/hnswlib) 和 [Annoy](https://github.com/spotify/annoy)。正如前面的教程中提到的，Milvus 是在这些库的基础上，提供一个完整的数据库，具有所有常见的数据库功能和一致的用户级 API。
...
不同类型的算法可能适用于不同类型的向量数据。虽然所有的算法都有助于加快向量搜索过程，不过这个过程中可能也会牺牲一些准确性。
...
平面索引大体上是最基本的索引策略，但也可以说是最容易被忽视的。通过平面索引，我们将一个查询向量与数据库中的其他所有向量进行比较。示例代码如下：
...
前两行代码在 1000 个元素的向量数据集之外创建了一个随机的查询向量。第三行用于是计算数据集中所有元素与查询向量之间的距离（**np.linalg.norm**），然后提取最小距离的索引（**np.argmin**）。这就给了我们与查询向量最近邻的数组索引，然后我们可以使用 **dataset[nearest,:]** 来提取数据。

这显然是执行向量搜索的最简单粗暴的方式。对于规模小的数据集来说，它的效果出奇的好，特别是如果你有一个加速器，如 GPU 或 FPGA 来并行处理搜索请求。比如，上面的循环在英特尔 i7-9750H CPU 上的运行时间不到 0.5 毫秒，这意味着我们可以在一个六核的笔记本上通过平面索引实现超过 2000 的 QPS。
...
如果熟悉 [FAISS](https://github.com/facebookresearch/faiss)，你应该对上图不会陌生。这是 Voronoi 图，直观地从两个维度展示了聚类分配。上图总共有 20 个单元（聚类），每个聚类的中心点显示为一个黑点。数据集中的所有点都将落入这 20 个区域中的一个。
...
**centroids** 现在包含了我们数据集的所有 **num_part **中心点（在 FAISS 中，这个参数被称为 **nlist**），而 **assignments** 包含了与每个向量最接近的中心点的 ID。我们可以通过以下方式来验证这一点：
...
上面的代码首先创建了一个列表，最外层对应的是 IVF 分片的数量。然后 for 循环迭代出所有的值，即每个向量属于哪个分片，并填充索引。

有了索引，我们现在可以通过只搜索最近的单元来限制整个搜索空间：

```python
>>> query = np.random.normal(size=(128,))
>>> c = np.argmin(np.linalg.norm(centroids - query, axis=1))  # 找到最近的分片
>>> nearest = np.argmin(np.linalg.norm(dataset[index[c]] - query, axis=1))  # 找到最近邻向量
>>> nearest
333
```

当 **num_part** 为16，数据集大小为 100K 的情况下，我们使用与之前相同的硬件（英特尔 i7-9750H CPU）得到了大约 150 QPS。将 **num_part** 提高到 64，我们得到了高达 650 的 QPS。

对高维数据来说，将搜索范围扩大到最近的聚类之外是可行的。如果你熟悉 FAISS，这里对应的是创建 IVF 索引时的 **nprobe** 参数。和二维或三维之类的数据相比，这主要是受维度的影响，每个分片的边的数量明显增多。至于如何设置 **nprobe**，目前还没有一个较好的经验法则。你可以先用数据进行实验，看看速度与准确性/召回的权衡，可能会比较有效。
...
下一个教程中，我们将继续深入研究标量量化和乘积量化的索引策略，这是 Milvus 受欢迎的两种量化策略。下次见！
