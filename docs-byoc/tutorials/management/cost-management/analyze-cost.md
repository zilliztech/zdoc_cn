...
notebook: FALSE
description: "Zilliz Cloud 的用量页为您提供可视化的成本分析能力，支持从多个维度查看 Zilliz Cloud 按量计费集群的使用和消费情况以及包年包月集群额外产生的存储和备份费用。 | BYOC"
type: origin
...
- **根据计费方式分析用量和成本**
    ...
    例如您在同一组织下创建了一个包年包月集群和一个按量计费集群。如果您需要计算包年包月集群的使用成本，您可以在计费类型筛选器中选择包年包月。筛选后，您可以查看所有包年包月集群额外产生的按量计费费用（即存储、备份等费用）。将筛选出的包年包月集群用量费用与包年包月集群的[订单](./manage-orders)费用相加后即可得出包年包月集群的总成本费用。
...
您还可以通过[查询日用量](https://docs.zilliz.com.cn/reference/restful/query-daily-usage-v2)的 API 接口来获取组织的用量信息。通过 API 接口获取的用量信息中，金额精度均为 8 位小数。如需对账，建议您通过 RESTful API 获取精度为 8 位小数的日用量明细。将每日用量的金额加总后，您将获取一个精度为 8 位的用量总金额，对该金额从第 3 位小数进行四舍五入后，您可以获得一个精度为 2 位小数的月用量总金额，该金额与界面上账单详情页展示的总用量金额一致。
