...
notebook: FALSE
description: "倒排索引（Inverted Index）是一种用于提高文本字段查询性能的索引技术，特别适合快速全文检索。其工作原理是将每个词汇映射到包含该词汇的文档列表（即倒排列表），从而在进行关键词搜索时，可以快速定位相关文档，大大减少检索时间。 | BYOC"
type: origin
...
创建倒排索引后，您可以在搜索查询中使用 `filter` 参数来基于索引字段执行标量过滤。这样可以利用倒排索引高效地缩小搜索结果范围。关于更多详细信息，请参考[过滤条件表达式](./undefined)。
