...
notebook: FALSE
description: "语言识别器（`languageidentifier`）是一种专用分词器，用于增强 Zilliz Cloud&lt;/zilliz&gt; 的文本搜索能力，它通过自动化语言分析流程来实现。其主要功能是检测文本字段的语言，然后动态应用最适合该语言的预配置分析器。这对处理多语言的应用尤为重要，因为它免去了逐条输入手动指定语言的麻烦。 | BYOC"
type: origin
