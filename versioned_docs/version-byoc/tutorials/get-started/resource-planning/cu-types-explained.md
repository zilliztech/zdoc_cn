...
下表从多个方面快速对比三种 CU 类型之间的差异。如需查看不同 CU 类型的具体容量和性能对比，请参考[选择最优的 CU 类型](./cu-types-explained#select-an-optimal-cu-type)。
...
- 适用于处理大量数据集，数据容量是性能型 CU 的 **4** 倍，但搜索效率相对较低。

- 特别适合大规模的非结构化数据检索、版权识别和身份验证。

### 存储扩展型 CU{#extended-capacity-cu}

- 适用于需要存储海量数据且追求性价比、对延时要求较低的应用。
...
<table>
   <tr>
     <th><p>向量维度</p></th>
     <th><p>性能型 CU（每 CU可容纳最大向量数）</p></th>
     <th><p>容量型 CU（每 CU可容纳最大向量数）</p></th>
     <th><p>存储扩展型 CU （（每 CU可容纳最大向量数）</p></th>
   </tr>
   <tr>
     <td><p>128</p></td>
     <td><p>750 万</p></td>
