...
## 数据组件{#data-components}{#data-components}

### 向量数量{#number-of-entities}{#number-of-entities}
...
### 向量维度{#vector-dimension}{#vector-dimension}

- 功能：向量中所包含的元素个数，也就是向量的长度。

- 调整范围：从 32 到 32,768 维。

### 标量字段{#scalar-fields}{#scalar-fields}

- 功能：添加数据的标量字段，以获得更精确的价格估算。

- 详情：为主键和其他字段选择数据类型。

## 定价变量{#pricing-variables}{#pricing-variables}

### CU 类型{#cu-type}{#cu-cu-type}
