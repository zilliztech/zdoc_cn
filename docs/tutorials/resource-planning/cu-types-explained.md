...
1. __时延评估__：性能型 CU 是唯一能满足 30 毫秒时延需求的类型。

1. __容量考量__：单个性能型 CU 能处理 200 万个 768 维向量。为容纳全部 800 万向量，您需配置至少 4 个此类 CU。

1. __吞吐量检验__：在 top-k 设置为100的情况下，性能型 CU 的每秒查询量（QPS）为 440。为维持 1000 QPS 的持续性能，需要将副本数量增加 3 倍。
