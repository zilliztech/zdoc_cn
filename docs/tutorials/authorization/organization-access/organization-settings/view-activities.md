...
Zilliz Cloud __事件__页面记录了[组织](./resource-hierarchy#understand-organizations)中发生的事件，包括免费额度增减、集群状态变更、授予或撤销组织和项目权限等。
...
1. 在左侧导航栏中，点击__事件__。在__事件__页面，您可以查看事件内容、事件发生时间以及对应的操作人。
...
- __根据时间筛选事件__
    ...
    如需查看特定时间段内发生的事件，请点击时间组件，选择起始日期和终止日期，点击__确认__。 
...
- __根据事件类型筛选__
    ...
    如需根据事件类型筛选，请点击事件类型下拉框并选择您想要查看的事件类型。Zilliz Cloud 中有 3 种事件类型：__信息__、__警告__和__错误__。
