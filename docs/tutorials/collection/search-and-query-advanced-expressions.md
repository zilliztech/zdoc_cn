...
- `count()`：用于检索 Collection 中的 Entity 数量。

- `json_contains()`：用于判断 JSON 键中是否包含特定元素。
...
创建并索引完 Collection 后，将示例数据集格式化以适应 Collection 的 Schema。以下示例展示了如何在 `article_meta` JSON 字段中增加 `tags_1` 和 `tags_2` 两个新行：
...
以上代码将查询同时满足以下两个条件的 Entity 数量：

- `article_meta` 中 `claps` 超过 30。
...
|  表达式                                       |  示例                                                                                                                               |  说明                                                                                                                                        |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
|  `json_contains` (`JSON_CONTAINS`)         |  `expr_1 = 'JSON_CONTAINS(article_meta["tags_1"], 16)'`<br/> `expr_2 = 'JSON_CONTAINS(article_meta["tags_2"], [5, 3, 39, 8])'` |  `expr_1` 在 `article_meta["tags_1"]` 包含元素 `16` 时返回 True。<br/> `expr_2`在 `article_meta["tags_2"]` 包含元素`[5, 3, 39, 8]` 时返回 True。<br/>  |
...
|  `json_contains_all` (`JSON_CONTAINS_ALL`) |  `expr_4 = 'JSON_CONTAINS_ALL(article_meta["tags_1"], [2, 4, 6])'`<br/>                                                        |  `expr_4` 在 `article_meta["tags_1"]` 包含 `[2, 4, 6]` 的所有元素时返回 True。<br/>                                                                 |
