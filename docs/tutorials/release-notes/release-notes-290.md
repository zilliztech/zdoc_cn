...
如需了解更多，可查看[使用 Sparse Vector](./use-sparse-vector) 一文及运行 [hello_sparse.py](https://github.com/milvus-io/pymilvus/blob/2.4/examples/hello_sparse.py) 中的示例代码。在运行示例代码前，须将代码中的连接信息指向您的 Zilliz Cloud 集群。
...
如需了解更多，可查阅[批量检索（Bulk-vector Search）](./single-vector-search#bulk-vector-search)和[Hybrid Search](./hybrid-search) 等内容及运行 [hybrid_search.py](https://github.com/milvus-io/pymilvus/blob/2.4/examples/hybrid_search.py) 中的示例代码。在运行示例代码前，须将代码中的连接信息指向您的 Zilliz Cloud 集群。
...
如需了解更多，可查阅[使用 Binary Vector](./use-binary-vector), [创建 Scalar Index](./index-scalar-fields) 以及[使用 ](./single-vector-search#single-vector-search#use-the-like-operator)`like`[ 操作符](./single-vector-search#single-vector-search#use-the-like-operator) 等内容。您还可以运行 [inverted_index_example.py](https://github.com/milvus-io/pymilvus/blob/2.4/examples/inverted_index_example.py) 和 [fuzzy_match.py](https://github.com/milvus-io/pymilvus/blob/2.4/examples/fuzzy_match.py) 中的示例代码深入了解相关概念。在运行示例代码前，须将代码中的连接信息指向您的 Zilliz Cloud 集群。
...
如需了解更多，可查阅 [Grouping Search](./single-vector-search#grouping-search) 等内容及运行 [hybrid_search.py](https://github.com/milvus-io/pymilvus/blob/2.4/examples/hybrid_search.py) 中的示例代码。在运行示例代码前，须将代码中的连接信息指向您的 Zilliz Cloud 集群。
...
如需了解更多，可查阅[支持的数据类型](./schema-explained#data-types)并运行 [float16_example.py](https://github.com/milvus-io/pymilvus/blob/2.4/examples/float16_example.py) 及 [bfloat16_example.py](https://github.com/milvus-io/pymilvus/blob/2.4/examples/bfloat16_example.py) 中的示例代码。在运行示例代码前，须将代码中的连接信息指向您的 Zilliz Cloud 集群。
