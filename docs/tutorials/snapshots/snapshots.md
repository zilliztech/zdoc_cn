...
beta: PRIVATE
notebook: FALSE
description: "快照是 Zilliz Cloud Collection 在特定时间点的镜像，适合用于快速回滚、版本管理和测试。快照会捕获 Collection 在指定时间戳的状态，并且只存储元数据和 Manifest 文件，例如 Schema、索引和向量数据文件（Binlog），从而实现高效存储和恢复。 | Cloud"
type: origin
