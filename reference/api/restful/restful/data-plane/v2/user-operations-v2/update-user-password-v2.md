...
curl --location --request POST "https://${CLUSTER_ENDPOINT}/v2/vectordb/users/update_password" \
...
| __password__ | __string__  <br/>要为新用户创建的对应密码。<br/>密码必须是8到64个字符的字符串，并且必须至少包含以下三种字符类型：大写字母、小写字母、数字和特殊字符。  |
| __newPassword__ | __string__  <br/>指定用户的新密码。   密码必须是8到64个字符的字符串，并且必须至少包含以下三种字符类型：大写字母、小写字母、数字和特殊字符。  |
