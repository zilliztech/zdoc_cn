...
<RestHeader method="post" endpoint="https://controller.api.${CLOUD_REGION_ID}.cloud.zilliz.com.cn/v1/clusters/createServerless" />
...
| 参数名称        | 参数描述                                                                               |
|------------------|-------------------------------------------------------------------------------------------|
| `plan`  | **string**（必选）<br/>待创建集群所属订阅计划。可选值有 `free` 和 `serverless`。|
| `clusterName`  | **string**（必选）<br/>待创建集群名称，长度不超过 32 个字符。|
...
| `data.username`   | **string**<br/>默认生成的集群用户名。 |
| `data.password`   | **string**<br/>默认生成的集群用户密码。此密码仅显示一次，请务必妥善保存。 |
| `data.prompt`   | **string**<br/>表明当前操作成功的提示信息。 |
