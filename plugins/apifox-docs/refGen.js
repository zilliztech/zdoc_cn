  ...
  async write_refs() {
    ...
    for (const page_url of Object.keys(specifications.paths)) {
      for (const method of Object.keys(specifications.paths[page_url])) {
        ...
        const condition = (page_slug.includes('cloud') || page_slug.includes('cluster') || page_slug.includes('import')) || page_slug.includes('pipeline') || page_slug.includes(('metrics'))
        const server = condition ? `https://controller.api.\${CLOUD_REGION_ID}.${host}` : "https://${CLUSTER_ENDPOINT}"
